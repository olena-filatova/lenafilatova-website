export default {
  "slug": "resveratrol-skin-ageing-women-over-40",
  "date": "2026-09-08",
  "image": "/images/resveratrol-skin-ageing-women-over-40.jpg",
  "cats": [
    "skin",
    "perimenopause"
  ],
  "en": {
    "cat": "Skin & beauty",
    "imageAlt": "A small glass jar of white cream and two capsules on a pale bathroom shelf beside a folded towel.",
    "title": "Resveratrol and wrinkles: what an 8-week trial in women over 40 actually found",
    "seoTitle": "Resveratrol and Wrinkles: What the 8-Week Trial Found",
    "metaDesc": "A placebo-controlled trial tested oral and topical trans-resveratrol in women over 40. The wrinkle result was small — the pigmentation result was bigger.",
    "excerpt": "A double-blind trial gave women over 40 resveratrol capsules, cream, both or neither for eight weeks. Only the group taking both beat placebo on wrinkles — and by less than the headlines suggest.",
    "meta": "8 September 2026 · 6 min read",
    "lead": "Resveratrol — the compound in red grape skin — has been sold as an anti-ageing ingredient for two decades on almost no human skin evidence. There is now a proper randomised trial. It is worth reading, and worth reading carefully.",
    "blocks": [
      {
        "t": "p",
        "text": "The trial, published in Frontiers in Aging in December 2025, randomised 134 healthy women aged 40 and over into four groups for eight weeks: capsules only, cream only, both, or neither. Everyone got something, and nobody — participants or assessors — knew which. That is a genuinely well-built design, and rarer in skincare than you would hope. 122 women finished."
      },
      {
        "t": "p",
        "text": "On wrinkles, only the group taking both the capsules and the cream did better than placebo, and the margin was modest. Wrinkle scores fell 11.9% in that group against 6.3% on placebo. In absolute terms that is a drop of 0.59 points versus 0.31 on a scale starting near 5 — a difference of under three tenths of a point after two months. Statistically real; visually, the sort of change you would struggle to see in the mirror. Capsules alone did nothing measurable to wrinkles at all."
      },
      {
        "t": "p",
        "text": "The result the coverage mostly skipped is more interesting. Both groups using the cream saw pigmentation improve significantly more than placebo, at a much stronger level of statistical confidence than the wrinkle finding. If you are dealing with uneven tone or dark patches rather than fine lines, that is the number in this trial that points at you. I have written separately about [why melasma and dark patches get worse in perimenopause](/blog/melasma-perimenopause/), and this fits that picture: topical actives have a better track record on pigment than on wrinkles."
      },
      {
        "t": "p",
        "text": "One widely repeated claim needs correcting. The cream did raise sebum — skin oil — substantially, roughly doubling it in the forehead-and-nose zone. That has been reported as good news for the drier skin of menopause. But the trial measured skin moisture separately, and moisture did not improve. Nor did elasticity, pore size or measured skin age. More oil is not the same as more hydration, and for anyone whose skin is oily or breakout-prone it is not obviously a benefit at all."
      },
      {
        "t": "p",
        "text": "Two caveats matter more than any of the numbers. The first is age: the women in this trial averaged 57 to 59, so this is evidence about post-menopausal skin, not about the 44-year-old in early perimenopause who is most likely to be reading about it. The second is funding. The trial was paid for by Evolva AG, which sells the resveratrol ingredient, and three of the authors work for the sponsor or its successor company. That does not make the result wrong — the design is sound and it was independently peer-reviewed — but industry-funded trials of the sponsor's own product return favourable results more often than independent ones do, and eight weeks is a short window in which to see a supplement fail."
      },
      {
        "t": "p",
        "text": "Where does that leave a bottle of resveratrol capsules? Roughly where the collagen evidence sits: not nothing, not much, and a long way behind the boring things that work. If you want to spend money on your skin after 40, sun protection and a retinoid still have far more behind them than any supplement in this category. If you want to try the cream for pigmentation specifically, that is the most defensible reading of this trial — and it is the cream doing the work, not the capsule."
      },
      {
        "t": "h",
        "text": "What the research says"
      },
      {
        "t": "refs",
        "items": [
          {
            "claim": "Only the combined oral-plus-topical group beat placebo on wrinkles, by a small margin.",
            "detail": "A double-blind, randomised, placebo-controlled trial published in Frontiers in Aging on 19 December 2025 randomised 134 healthy women aged 40+ (mean age 57.6–59.3 across groups; 122 completed) to four arms for eight weeks: placebo oral + placebo topical, active oral + placebo topical, placebo oral + active topical, or active both. Dosing was 75 mg trans-resveratrol orally twice daily and a 1.5% trans-resveratrol cream, 1 g applied twice daily. At week 8, the both-active group's wrinkle score fell significantly more than placebo (−0.59, or −11.9%, versus −0.31, or −6.3%; p < 0.05) from a baseline of approximately 5.0. Oral-only did not differ from placebo on wrinkles. Products were reported as safe with only mild adverse events.",
            "cite": "Frontiers in Aging · Randomised controlled trial, 19 December 2025",
            "url": "https://www.frontiersin.org/journals/aging/articles/10.3389/fragi.2025.1727244/full"
          },
          {
            "claim": "The cream improved pigmentation more strongly than it improved wrinkles.",
            "detail": "In the same trial, both groups using the active cream showed significantly greater reduction in pigmentation than the double-placebo group (p < 0.001) — a stronger significance level than the wrinkle finding. U-zone sebum also rose sharply in the topical-active groups (increases of 88.7% and 108.7% from baseline, versus 34.7% and 22.2% in the topical-placebo groups; p < 0.05). However, the authors report no difference between groups at any time point for skin moisture, elasticity, pore size, T-zone sebum, skin temperature or measured skin age — so the increase in oil was not accompanied by an increase in hydration.",
            "cite": "Frontiers in Aging · Randomised controlled trial, 19 December 2025",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC12757695/"
          },
          {
            "claim": "The trial was funded by the company selling the ingredient.",
            "detail": "The study was funded by Evolva AG (Reinach, Switzerland), the supplier of the trans-resveratrol used. Two authors were employed by Lallemand Bio-Ingredients, the successor company, and one by Evolva; the trial was conducted by RDC Clinical in Brisbane. This is disclosed in the paper and does not invalidate a peer-reviewed, double-blind design, but sponsor-funded trials of a sponsor's own product report favourable outcomes more frequently than independently funded trials, and it is a relevant consideration when weighing an effect of this size.",
            "cite": "Frontiers in Aging · Funding and conflict-of-interest statement, 2025",
            "url": "https://www.frontiersin.org/journals/aging/articles/10.3389/fragi.2025.1727244/full"
          }
        ]
      },
      {
        "t": "links",
        "title": "Related reading",
        "items": [
          {
            "name": "Why melasma and dark patches get worse in perimenopause",
            "url": "/blog/melasma-perimenopause/",
            "desc": "The pigmentation problem this trial's strongest result speaks to."
          },
          {
            "name": "Vitamin C serum after 40 — does it earn its place?",
            "url": "/blog/vitamin-c-serum-over-40/",
            "desc": "Another topical antioxidant, and how its evidence compares."
          },
          {
            "name": "Longevity marketing has found menopausal skin",
            "url": "/blog/menopause-skincare-longevity-marketing/",
            "desc": "How a modest trial result becomes a marketing claim."
          }
        ]
      }
    ],
    "faq": [
      {
        "q": "Do resveratrol supplements reduce wrinkles?",
        "a": "On the best evidence available, barely. In an eight-week randomised trial in women aged 40 and over, capsules taken on their own made no measurable difference to wrinkles. Only the group using both the capsules and a 1.5% resveratrol cream did better than placebo, and the gap was under three tenths of a point on a wrinkle score — real in the statistics, hard to see in a mirror. The trial was also funded by the company that supplies the ingredient."
      },
      {
        "q": "Is resveratrol cream good for dry skin in menopause?",
        "a": "That claim goes further than the data. The cream did roughly double sebum — the skin's own oil — in the forehead and nose area. But the same trial measured hydration separately and found no improvement in skin moisture, and none in elasticity either. Oil and moisture are not the same thing. The clearest benefit the cream showed was on pigmentation and uneven tone, not on dryness."
      }
    ]
  },
  "ua": {
    "cat": "Краса та шкіра",
    "imageAlt": "Невелика скляна баночка з білим кремом і дві капсули на світлій полиці у ванній поруч зі складеним рушником.",
    "title": "Ресвератрол і зморшки: що насправді показало 8-тижневе дослідження в жінок після 40",
    "seoTitle": "Ресвератрол і зморшки: що показало дослідження",
    "metaDesc": "Плацебо-контрольоване дослідження перевірило ресвератрол у капсулах і кремі в жінок після 40. Ефект на зморшки малий — на пігментацію більший.",
    "excerpt": "У подвійному сліпому дослідженні жінки після 40 вісім тижнів приймали ресвератрол у капсулах, крем, і те й те — або плацебо. Плацебо переграла лише група, що вживала обидва засоби, і з меншим ефектом, ніж обіцяють заголовки.",
    "meta": "8 вересня 2026 · 6 хв читання",
    "lead": "Ресвератрол — сполуку зі шкірки червоного винограду — продають як антиейдж-інгредієнт уже два десятиліття, майже без доказів на людській шкірі. Тепер є повноцінне рандомізоване дослідження. Його варто прочитати — і прочитати уважно.",
    "blocks": [
      {
        "t": "p",
        "text": "Дослідження, опубліковане у Frontiers in Aging у грудні 2025 року, рандомізувало 134 здорові жінки віком від 40 років на чотири групи на вісім тижнів: лише капсули, лише крем, і те й те, або нічого активного. Кожна отримувала якийсь засіб, і ніхто — ні учасниці, ні ті, хто оцінював результат, — не знав який. Це справді добре побудований дизайн, і в косметології він трапляється рідше, ніж хотілося б. Дослідження завершили 122 жінки."
      },
      {
        "t": "p",
        "text": "Щодо зморшок плацебо переграла лише група, яка вживала і капсули, і крем, — і перевага була скромною. Показник зморшок у ній знизився на 11,9% проти 6,3% на плацебо. В абсолютних числах це −0,59 бала проти −0,31 за шкалою, що починалася приблизно з 5, тобто різниця менша за три десятих бала за два місяці. Статистично реально; візуально — така зміна, яку важко помітити в дзеркалі. Самі лише капсули на зморшки не вплинули взагалі."
      },
      {
        "t": "p",
        "text": "Результат, який здебільшого проігнорували в новинах, цікавіший. В обох групах, що користувалися кремом, пігментація зменшилася значно більше, ніж на плацебо, — і з набагато вищим рівнем статистичної впевненості, ніж результат щодо зморшок. Якщо ваша проблема — нерівний тон і темні плями, а не дрібні зморшки, то саме це число в дослідженні стосується вас. Я окремо писала про те, [чому мелазма й темні плями посилюються в перименопаузі](/ua/blog/melasma-perimenopause/), і це вкладається в ту саму картину: у зовнішніх засобів кращий послужний список щодо пігменту, ніж щодо зморшок."
      },
      {
        "t": "p",
        "text": "Одне поширене твердження потребує виправлення. Крем справді помітно підвищив себум — шкірне сало — приблизно вдвічі в зоні чола й носа. Це подавали як добру новину для сухішої шкіри в менопаузі. Але дослідження окремо вимірювало вологість шкіри, і вона не покращилася. Як і еластичність, розмір пор чи виміряний «вік шкіри». Більше жиру — це не більше вологи, а для тих, у кого шкіра жирна або схильна до висипань, це взагалі сумнівна перевага."
      },
      {
        "t": "p",
        "text": "Два застереження важать більше за будь-які цифри. Перше — вік: середній вік учасниць був 57–59 років, тож це дані про постменопаузальну шкіру, а не про 44-річну жінку на початку перименопаузи, яка найімовірніше про це й читає. Друге — фінансування. Дослідження оплатила компанія Evolva AG, яка продає цей інгредієнт ресвератролу, і троє авторів працюють на спонсора або його компанію-наступницю. Це не робить результат хибним — дизайн надійний, робота пройшла рецензування, — але дослідження, профінансовані виробником власного продукту, дають сприятливі результати частіше за незалежні, а вісім тижнів — короткий строк, щоб добавка встигла показати невдачу."
      },
      {
        "t": "p",
        "text": "То що це означає для баночки капсул із ресвератролом? Приблизно те саме, що й доказова база щодо колагену: не нуль, але й небагато, і далеко позаду нудних речей, які працюють. Якщо ви хочете витратити гроші на шкіру після 40, за захистом від сонця й ретиноїдом стоїть значно більше доказів, ніж за будь-якою добавкою цієї категорії. Якщо хочете спробувати крем саме проти пігментації — це найбільш обґрунтоване прочитання цього дослідження. І працює саме крем, а не капсула."
      },
      {
        "t": "h",
        "text": "Що каже наука"
      },
      {
        "t": "refs",
        "items": [
          {
            "claim": "Плацебо переграла лише група, що поєднувала капсули й крем, — і з невеликою перевагою.",
            "detail": "Подвійне сліпе рандомізоване плацебо-контрольоване дослідження, опубліковане у Frontiers in Aging 19 грудня 2025 року, рандомізувало 134 здорові жінки віком від 40 років (середній вік у групах 57,6–59,3; завершили 122) на чотири групи на вісім тижнів: плацебо всередину + плацебо зовнішньо, активна речовина всередину + плацебо зовнішньо, плацебо всередину + активна зовнішньо, або активна речовина в обох формах. Дозування: 75 мг транс-ресвератролу всередину двічі на добу і крем із 1,5% транс-ресвератролу, 1 г двічі на добу. На 8-му тижні показник зморшок у групі з обома активними засобами знизився значно більше, ніж на плацебо (−0,59, або −11,9%, проти −0,31, або −6,3%; p < 0,05) від початкового рівня близько 5,0. Самі лише капсули від плацебо за зморшками не відрізнялися. Засоби визнано безпечними, побічні явища — лише легкі.",
            "cite": "Frontiers in Aging · Рандомізоване контрольоване дослідження, 19 грудня 2025",
            "url": "https://www.frontiersin.org/journals/aging/articles/10.3389/fragi.2025.1727244/full"
          },
          {
            "claim": "Крем покращив пігментацію сильніше, ніж зморшки.",
            "detail": "У тому самому дослідженні в обох групах, що користувалися активним кремом, пігментація зменшилася значно більше, ніж у групі подвійного плацебо (p < 0,001) — це вищий рівень значущості, ніж у результату щодо зморшок. Себум у Т-зоні чола й носа теж різко зріс у групах з активним кремом (на 88,7% і 108,7% від початкового рівня проти 34,7% і 22,2% у групах із плацебо-кремом; p < 0,05). Проте автори не виявили різниці між групами в жодній точці спостереження за вологістю шкіри, еластичністю, розміром пор, себумом Т-зони, температурою шкіри чи виміряним «віком шкіри» — тобто зростання жиру не супроводжувалося зростанням зволоженості.",
            "cite": "Frontiers in Aging · Рандомізоване контрольоване дослідження, 19 грудня 2025",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC12757695/"
          },
          {
            "claim": "Дослідження профінансувала компанія, що продає цей інгредієнт.",
            "detail": "Дослідження профінансувала Evolva AG (Райнах, Швейцарія) — постачальник використаного транс-ресвератролу. Двоє авторів працювали в Lallemand Bio-Ingredients, компанії-наступниці, ще один — в Evolva; дослідження провела RDC Clinical у Брисбені. Це розкрито в статті й не скасовує рецензованого подвійного сліпого дизайну, але дослідження, профінансовані виробником власного продукту, частіше повідомляють про сприятливі результати, ніж незалежні, і це варто враховувати, оцінюючи ефект такого розміру.",
            "cite": "Frontiers in Aging · Заява про фінансування та конфлікт інтересів, 2025",
            "url": "https://www.frontiersin.org/journals/aging/articles/10.3389/fragi.2025.1727244/full"
          }
        ]
      },
      {
        "t": "links",
        "title": "Читайте також",
        "items": [
          {
            "name": "Чому мелазма й темні плями посилюються в перименопаузі",
            "url": "/ua/blog/melasma-perimenopause/",
            "desc": "Проблема пігментації, якої стосується найсильніший результат цього дослідження."
          },
          {
            "name": "Сироватка з вітаміном С після 40 — чи варта вона свого місця?",
            "url": "/ua/blog/vitamin-c-serum-over-40/",
            "desc": "Ще один зовнішній антиоксидант — і як виглядає його доказова база."
          },
          {
            "name": "Маркетинг довголіття дістався менопаузальної шкіри",
            "url": "/ua/blog/menopause-skincare-longevity-marketing/",
            "desc": "Як скромний результат дослідження перетворюється на рекламну обіцянку."
          }
        ]
      }
    ],
    "faq": [
      {
        "q": "Чи зменшують добавки з ресвератролом зморшки?",
        "a": "За найкращими наявними доказами — ледь-ледь. У восьмитижневому рандомізованому дослідженні в жінок від 40 років самі лише капсули не дали жодної помітної різниці щодо зморшок. Плацебо переграла лише група, що поєднувала капсули з кремом із 1,5% ресвератролу, і різниця склала менш ніж три десятих бала за шкалою зморшок — реальна в статистиці, важко помітна в дзеркалі. До того ж дослідження профінансувала компанія, яка постачає цей інгредієнт."
      },
      {
        "q": "Чи корисний крем із ресвератролом для сухої шкіри в менопаузі?",
        "a": "Це твердження йде далі, ніж дозволяють дані. Крем справді приблизно вдвічі підвищив себум — власне шкірне сало — у зоні чола й носа. Але те саме дослідження окремо вимірювало зволоженість і не виявило покращення вологості шкіри, як і еластичності. Жир і волога — це не одне й те саме. Найчіткіша користь, яку показав крем, стосувалася пігментації та нерівного тону, а не сухості."
      }
    ]
  }
};
