// Same date as 1 earlier post; seq keeps the hub order stable.
export const seq = 1;

export default {
  "slug": "glp1-what-we-know",
  "date": "2026-08-21",
  "featured": true,
  "image": "/images/glp1-what-we-know.jpg",
  "cats": [
    "glp1",
    "weight",
    "perimenopause",
    "diabetes"
  ],
  "en": {
    "cat": "Weight loss",
    "imageAlt": "A close-up of someone giving themselves an injection in the abdomen with a pen injector.",
    "title": "GLP-1 drugs after 40: what we actually know, and why the headlines keep contradicting each other",
    "seoTitle": "GLP-1 Drugs After 40: What the Evidence Shows",
    "metaDesc": "Seven GLP-1 studies read side by side. Most contradictions are differences in who was measured — all in one place, with how strong each finding is.",
    "excerpt": "One study says GLP-1 drugs burn fat, not muscle; another, that a quarter to 40% of the loss is muscle. They measured different women, by different methods. Here’s all of it.",
    "meta": "21 August 2026 · 10 min read",
    "lead": "I have written about GLP-1 drugs seven times in six weeks, and if you read those posts back to back you would be forgiven for thinking the research is a mess. One says the weight coming off is mostly fat. The next says a quarter to 40% of it is muscle. A third says that losing the weight costs you bone that you will not get back. They sound like they are contradicting each other. Almost none of them actually are — and the reason why is more useful than any single finding.",
    "blocks": [
      {
        "t": "p",
        "text": "There are two things going on, and once you can see them the disagreements mostly dissolve."
      },
      {
        "t": "p",
        "text": "The first is who was measured. A trial of adults with obesity in their late forties and a review of women aged 65 and over are not describing the same body. The same drug can produce a reassuring number in one and a worrying number in the other without either being wrong."
      },
      {
        "t": "p",
        "text": "The second is how strong the study was. Some of what follows is a meta-analysis of dozens of randomised trials. Some of it is one researcher’s reading of older data. One of them is a conference presentation that has not been peer-reviewed. These do not deserve equal weight, and in the coverage they usually get it."
      },
      {
        "t": "h",
        "text": "Everything on this site about GLP-1s, in one table"
      },
      {
        "t": "p",
        "text": "Read down the third column before the first. It is the one that tells you how much to bet on any given row."
      },
      {
        "t": "table",
        "head": [
          "What it found",
          "Who it was measured in",
          "How much weight to give it"
        ],
        "rows": [
          [
            "Most of the weight lost is fat, not muscle",
            "Adults with overweight or obesity, with or without type 2 diabetes — general trial populations",
            "High — systematic review pooling 24 randomised trials"
          ],
          [
            "Lean mass as a share of your body actually goes up",
            "Adults with obesity, 7 trials, 821 people",
            "High — meta-analysis of randomised trials, though with very high statistical heterogeneity"
          ],
          [
            "Women lose more than men — 10.9% of body weight against 6.8%",
            "6 trials, 19,906 adults, the only ones that reported by sex",
            "Moderate — from a 64-trial meta-analysis, but only 6 of those trials split by sex, and the confidence intervals overlap"
          ],
          [
            "A quarter to 40% of what comes off is lean mass",
            "Women aged 65 and over",
            "Moderate to low — narrative review; the trials were too varied in design to pool statistically"
          ],
          [
            "Adding HRT to tirzepatide gave 35% more weight loss",
            "Postmenopausal women already taking tirzepatide",
            "Low — retrospective cohort; an association, not proof of cause"
          ],
          [
            "Weight loss means bone loss, and the bone does not come back",
            "Postmenopausal women, extrapolated from a 2006 exercise study",
            "Low — one expert’s reading of existing data. No trial has tested GLP-1s and bone directly"
          ],
          [
            "People moved 560 fewer steps a day after starting",
            "753 US adults with obesity wearing activity trackers",
            "Lowest here — conference presentation, not yet peer-reviewed"
          ],
          [
            "GLP-1s entered the type 1 diabetes guidelines",
            "Adults with type 1 diabetes and a BMI of 30+",
            "Guideline consensus, built mostly on liraglutide trials with modest effects"
          ]
        ]
      },
      {
        "t": "h",
        "text": "How much weight actually comes off"
      },
      {
        "t": "p",
        "text": "The largest body-composition analysis to date pooled 24 randomised trials and found average body weight down about 9% at three months, 5% at six months and 4% at twelve. Those descending numbers look alarming until you notice what they are: separate groups of trials reporting at each duration, not one set of people followed across a year. They are not evidence that the drugs stop working. They mostly reflect which trials chose to report when, and how much starting weight differed between them."
      },
      {
        "t": "p",
        "text": "The other big number is the one about women, and it is worth being precise because it has been badly reported. A Johns Hopkins team pooled 64 randomised trials and found that women lost 10.9% of their body weight against 6.8% for men. You will see that written up as women losing “60% more”, which is arithmetically true and wildly oversells it — in absolute terms the gap is about four percentage points of body weight. Two further caveats matter more than the headline. Only 6 of those 64 trials reported results separately by sex at all, and the confidence intervals around the two figures overlap (7.0–14.8% for women, 4.6–9.0% for men). And the researchers’ own emphasis was almost the opposite of how it travelled: their main finding was that these drugs work about equally well regardless of age, race, ethnicity, starting BMI or starting HbA1c. Sex was the single exception in the entire analysis. [The full write-up is here](/blog/glp1-weight-loss-sex-difference-women-men/)."
      },
      {
        "t": "p",
        "text": "One exclusion matters if you are choosing between drugs: that analysis deliberately left out tirzepatide, sold as Mounjaro and Zepbound, because it acts on a second gut hormone as well as GLP-1. None of those numbers describe it."
      },
      {
        "t": "h",
        "text": "The muscle question, which is where the real confusion lives"
      },
      {
        "t": "p",
        "text": "This is the one that generates the contradictory headlines, and it has a clean resolution once you notice that the studies are answering three different questions."
      },
      {
        "t": "p",
        "text": "Do you lose muscle in absolute terms? Yes. A meta-analysis of seven randomised trials in 821 people found absolute lean mass fell by about 1.74 kg on average, and by considerably more on semaglutide specifically. In women aged 65 and over, [a review put the lean-mass share of total weight loss at roughly a quarter to 40%](/blog/glp1-older-women-lean-mass/). In the SEMALEAN study — 106 patients, mostly women — absolute lean mass dropped around 3 kg in the early months."
      },
      {
        "t": "p",
        "text": "Does muscle fall as a proportion of your body? No — it rises. The same seven-trial meta-analysis found lean mass as a share of total body weight increased by 1.81%. That is not a contradiction of the paragraph above; it is what happens when you lose a lot of fat and a little muscle at the same time. You end up with fewer kilograms of muscle on a body that is a larger percentage muscle than it was."
      },
      {
        "t": "p",
        "text": "Does your muscle work worse? In the one study that looked, no. In SEMALEAN, the share of participants with sarcopenic obesity — too much fat and too little muscle at once — fell from 49% at the start to 33% after twelve months. The early lean-mass decline stabilised over follow-up, and measures of muscle function improved. That study’s authors concluded that lean-mass loss should not by itself be a reason to withhold these drugs."
      },
      {
        "t": "p",
        "text": "So [“mostly fat, not muscle”](/blog/glp1-weight-loss-drugs-fat-not-muscle/) and “a quarter to 40% is muscle” are the same finding read from opposite ends. A quarter to 40% still leaves the majority coming off as fat. Which end matters to you depends almost entirely on how much muscle you had when you started — which is the honest reason the reassuring version comes from general adult trial populations and the worrying one comes from women aged 65 and over."
      },
      {
        "t": "h",
        "text": "Bone, which gets a fraction of the attention and has the thinnest evidence"
      },
      {
        "t": "p",
        "text": "Wendy Kohrt has studied bone and exercise for 35 years, and her position is blunt: weight loss means bone loss. Her own 2006 study is the uncomfortable part. Women on a supervised six-month exercise programme lost about ten pounds, and unusually the loss was fat rather than muscle — and they still lost more bone than women who had not lost weight. Followed for another year, neither those who regained weight nor those who did not regained any bone density."
      },
      {
        "t": "p",
        "text": "Set against the fact that women aged 50 to 64 are the heaviest users of these drugs, and that as many as two-thirds of people who start one stop within a year and regain much of the weight, her concern is that a lot of women are being put through repeated loss-and-regain cycles that subtract bone each time without adding it back. Her practical suggestion is a baseline DXA scan before starting. [The full argument, and its limits, are here](/blog/glp1-menopause-bone-loss/)."
      },
      {
        "t": "p",
        "text": "Be clear about what this is, though: an expert reading across existing data, not a trial. No study has tested GLP-1 drugs and bone density directly in this group, and a scoping review published in January concluded that larger studies are needed before anything can be said. It is the weakest evidence on this page and the most consequential question on it, which is an uncomfortable combination rather than a reason to dismiss it."
      },
      {
        "t": "h",
        "text": "If you are in perimenopause or past it"
      },
      {
        "t": "p",
        "text": "Two findings speak to this directly, and both come with a caution. Postmenopausal women taking hormone therapy alongside tirzepatide [lost about 35% more weight](/blog/tirzepatide-hrt-weight-loss-menopause-2026/) than those on tirzepatide alone — −19.2% against −14.0% over twelve months. That is a striking number from a retrospective cohort study, which means it shows an association and cannot rule out that women who choose hormone therapy differ in other ways that also help with weight."
      },
      {
        "t": "p",
        "text": "The second is a hypothesis rather than a finding. Among the explanations the Johns Hopkins authors offered for the sex difference was a possible synergy with oestrogen. If that is what drives it, it raises a question nobody has answered: does the female advantage survive the fall in oestrogen at menopause? There is no data either way, and it is worth knowing that the gap in the evidence is exactly where most readers of this site are standing."
      },
      {
        "t": "p",
        "text": "Read through Kohrt’s lens, more weight lost is also not straightforwardly good news for a skeleton — though oestrogen itself protects bone, which pulls the other way. Both things are true and nobody has run the study that settles it."
      },
      {
        "t": "h",
        "text": "And if you have type 1 diabetes"
      },
      {
        "t": "p",
        "text": "The 2026 ADA Standards of Care endorse GLP-1-based therapy for managing obesity in adults with type 1 diabetes and a BMI of 30 or above, for the first time. That is a weight-management recommendation sitting on top of insulin, not a replacement for it, and the drugs are still not licensed specifically for type 1. The underlying evidence is mostly liraglutide, with modest average effects, and the guideline flags a real risk of ketoacidosis occurring even when glucose looks near-normal. [What changed and what it means for women](/blog/ada-2026-glp1-type1-diabetes-women/)."
      },
      {
        "t": "h",
        "text": "The one thing every single paper agrees on"
      },
      {
        "t": "p",
        "text": "This is the part I would keep if you forget everything else. The body-composition meta-analysis, the review of older women, the bone researcher and the type 1 guideline all arrive at the same two levers: resistance training and enough protein. Not cardio, not the drug dose, not a supplement. The review of women over 65 is the most specific — resistance work two to three times a week totalling around 150 minutes, and 1.2 to 1.5 g of protein per kilogram of body weight per day, spread evenly across meals rather than loaded into one. Both of those sit above standard adult guidance, and they are the authors’ recommendations drawn from the wider literature rather than the result of a trial testing the combined protocol against the drug alone."
      },
      {
        "t": "p",
        "text": "Which makes one more finding awkward. Tracking 753 adults’ wearables, researchers found that after starting a GLP-1, average daily steps fell by about 560 and time in moderate-to-vigorous exercise dropped from roughly 28 minutes to 22. That is the weakest study here — a conference presentation, not peer-reviewed — and it describes an average, not a rule. But it points the wrong way at exactly the moment the evidence says movement matters most. [The detail is here](/blog/glp1-drugs-less-movement/). The drug reduces appetite; it does not build strength, and nothing in this literature suggests the training takes care of itself."
      },
      {
        "t": "h",
        "text": "How to read the next GLP-1 headline"
      },
      {
        "t": "p",
        "text": "Three questions, in this order, and they will do most of the work."
      },
      {
        "t": "p",
        "text": "Who was measured? Trial populations skew middle-aged. If a finding is about women over 65, or postmenopausal women specifically, it will not match the general numbers — and if a headline does not say, assume general."
      },
      {
        "t": "p",
        "text": "What kind of study was it? A meta-analysis of randomised trials, a narrative review, an observational cohort and a conference abstract are four very different things wearing the same word, “study”."
      },
      {
        "t": "p",
        "text": "A percentage of what? “Women lose 60% more” and “women lose four percentage points more” describe the same result. So do “most of it is fat” and “a third of it is muscle”."
      },
      {
        "t": "p",
        "text": "None of this is an argument for or against taking one. It is an argument for knowing which question a given number answers — and for treating the two levers everybody agrees on as part of the treatment rather than an optional extra."
      },
      {
        "t": "h",
        "text": "What the research says"
      },
      {
        "t": "refs",
        "items": [
          {
            "claim": "Across 24 pooled randomised trials, most weight lost on GLP-1 drugs was fat rather than lean tissue.",
            "detail": "A systematic review and meta-analysis in the International Journal of Obesity searched to December 2024, reviewed 36 studies and pooled 24, with subgroups by drug type and treatment duration. Mean body weight fell approximately 9% at three months, 5% at six and 4% at twelve. Fat mass decline predominated and lean body mass reductions were modest. It reports no sex-stratified results.",
            "cite": "International Journal of Obesity · Systematic review and meta-analysis, 25 April 2026 (PMID 42034831)",
            "url": "https://pubmed.ncbi.nlm.nih.gov/42034831/"
          },
          {
            "claim": "Lean mass rose as a proportion of body weight (+1.81%) while falling in absolute terms (−1.74 kg).",
            "detail": "A meta-analysis of seven randomised controlled trials in 821 patients with obesity treated at obesity doses versus placebo. Lean mass as a proportion of total weight increased 1.81% (95% CI 1.1–2.52). Absolute lean mass fell 1.74 kg (95% CI −3.04 to −0.45) and the percentage of lean mass fell 3.06%, both with very high heterogeneity (I²=98%); semaglutide showed the largest absolute reduction at −5.44 kg. The authors conclude lean-mass loss should not be considered a limitation on using these drugs, provided treatment is paired with nutrition and exercise.",
            "cite": "International Journal of Obesity · Systematic review and meta-analysis, 19 June 2026 (PMID 42321502)",
            "url": "https://pubmed.ncbi.nlm.nih.gov/42321502/"
          },
          {
            "claim": "Women lost 10.9% of body weight against 6.8% for men — with overlapping confidence intervals, from 6 of 64 trials.",
            "detail": "A Johns Hopkins systematic review and meta-analysis in JAMA Internal Medicine searched to 26 July 2024 and included 41 articles representing 64 randomised trials. Among the 6 trials (19,906 patients) analysed by sex, weight loss was greater in women (10.9%; 95% CI 7.0–14.8) than men (6.8%; 95% CI 4.6–9.0). Only 10 of the 48 individually characterised trials evaluated sex at all. No significant heterogeneity of treatment effect was found by age, race, ethnicity, baseline BMI or baseline HbA1c. Tirzepatide was not included.",
            "cite": "JAMA Internal Medicine · Systematic review and meta-analysis, 2026 (PMID 41770554)",
            "url": "https://pubmed.ncbi.nlm.nih.gov/41770554/"
          },
          {
            "claim": "In women aged 65 and over, roughly 25–40% of total weight loss was attributed to lean mass.",
            "detail": "A narrative review in Nutrients synthesising phase 3 trial and real-world data in older women. Women aged ≥65 achieved sustained weight loss of 10–20%. Approximately 25–40% of total weight loss was attributed to lean mass reduction. In the SEMALEAN study (106 patients, predominantly female) absolute lean mass fell about 3 kg early then stabilised, muscle function improved, and sarcopenic obesity prevalence fell from 49% at baseline to 33% at 12 months. The review recommends resistance exercise 2–3 times weekly (about 150 minutes) and 1.2–1.5 g of protein per kg of body weight daily, distributed across meals. Evidence was synthesised narratively because study designs were too heterogeneous to pool.",
            "cite": "Nutrients · Narrative review, 14 February 2026 (PMID 41754149)",
            "url": "https://pubmed.ncbi.nlm.nih.gov/41754149/"
          },
          {
            "claim": "Weight loss causes bone loss even when the weight lost is fat — and the bone does not return when the weight does.",
            "detail": "Wendy Kohrt (University of Colorado Anschutz) describing her 2006 supervised-exercise study and its one-year follow-up, in which neither participants who regained weight nor those who did not regained bone density. She reports that about 15% of US adult women have used a GLP-1, with use highest among those aged 50 to 64. This is an expert interview reading across existing data, not a trial of GLP-1s and bone.",
            "cite": "UCHealth Today · Expert interview, 22 June 2026",
            "url": "https://www.uchealth.org/today/menopause-and-glp-1-weight-loss-drugs-and-bone-loss/"
          },
          {
            "claim": "Adding hormone therapy to tirzepatide was associated with 35% more weight loss after menopause.",
            "detail": "A retrospective cohort study of postmenopausal women on tirzepatide, comparing those also using menopausal hormone therapy with those not. Total bodyweight reduction was −19.2% against −14.0% over 12 months (p=0.0023). Observational: it shows an association and cannot establish cause.",
            "cite": "The Lancet Obstetrics, Gynaecology & Women’s Health · Retrospective cohort study, 2026",
            "url": "https://www.thelancet.com/journals/lanogw/article/PIIS3050-5038(25)00145-1/abstract"
          },
          {
            "claim": "The 2026 ADA Standards endorse GLP-1-based therapy for obesity in adults with type 1 diabetes.",
            "detail": "First inclusion of GLP-1 receptor agonist-based therapy as an option for obesity management in adults with type 1 diabetes and a BMI of 30 or above (27.5 for Asian Americans). Expert consensus built on pooled evidence; the largest type 1 trials used liraglutide 1.8 mg with modest average effects, and the guideline flags risks of hypoglycaemia and euglycaemic ketoacidosis.",
            "cite": "American Diabetes Association · Standards of Care in Diabetes—2026, Section 8",
            "url": "https://diabetesjournals.org/care/article/49/Supplement_1/S166/163915/8-Obesity-and-Weight-Management-for-the-Prevention"
          },
          {
            "claim": "Daily steps fell by about 560 after starting a GLP-1, and moderate-to-vigorous exercise from 28 to 22 minutes.",
            "detail": "Wearable-tracker data from 753 adults with obesity in the NIH All of Us programme, comparing activity before and after starting a GLP-1. Declines were steepest in men and in people with musculoskeletal pain. Presented at a conference and not yet peer-reviewed.",
            "cite": "Endocrine Society (ENDO 2026) · Conference presentation",
            "url": "https://www.healio.com/news/endocrinology/20260613/exercise-declined-in-patients-with-obesity-after-starting-a-glp1"
          }
        ]
      },
      {
        "t": "links",
        "title": "Every GLP-1 post on this site",
        "items": [
          {
            "name": "Weight loss means bone loss — the case for caution on GLP-1s in menopause",
            "url": "/blog/glp1-menopause-bone-loss/",
            "desc": "The bone question, and why the evidence for it is thinner than the concern deserves."
          },
          {
            "name": "GLP-1 drugs in women over 65 — how much of the loss is muscle",
            "url": "/blog/glp1-older-women-lean-mass/",
            "desc": "Where the quarter-to-40% figure comes from, and the protocol the authors propose instead of just warning about it."
          },
          {
            "name": "Women lose more weight on GLP-1 drugs than men",
            "url": "/blog/glp1-weight-loss-sex-difference-women-men/",
            "desc": "64 trials — and the researchers’ actual emphasis was how little else made any difference."
          },
          {
            "name": "Most of the weight lost on a GLP-1 is fat, not muscle",
            "url": "/blog/glp1-weight-loss-drugs-fat-not-muscle/",
            "desc": "The reassuring number, and the general adult trial populations it was measured in."
          },
          {
            "name": "Hormone therapy plus tirzepatide: 35% more weight loss after menopause",
            "url": "/blog/tirzepatide-hrt-weight-loss-menopause-2026/",
            "desc": "A striking result from an observational study, with the caveat that entails."
          },
          {
            "name": "The 2026 diabetes guidelines back GLP-1 drugs in type 1 diabetes",
            "url": "/blog/ada-2026-glp1-type1-diabetes-women/",
            "desc": "The guideline change, and why it lands differently for women with type 1."
          },
          {
            "name": "People on GLP-1 drugs moved less, not more",
            "url": "/blog/glp1-drugs-less-movement/",
            "desc": "The wearable-data finding that complicates every ‘just add resistance training’ recommendation."
          },
          {
            "name": "Eating more protein than the official guideline supports muscle and bone after menopause",
            "url": "/blog/higher-protein-postmenopausal-bone-muscle/",
            "desc": "The protein half of the advice every one of these papers converges on."
          },
          {
            "name": "Starting resistance training in perimenopause",
            "url": "/blog/strength-training-perimenopause-beginners/",
            "desc": "The training half, from a standing start."
          }
        ]
      }
    ],
    "faq": [
      {
        "q": "Do GLP-1 drugs make you lose muscle, or not?",
        "a": "Both answers are in the evidence because they measure different things. In absolute terms yes — a meta-analysis of seven trials found lean mass fell by about 1.74 kg on average. As a proportion of your body, no: the same analysis found lean mass rose by 1.81% as a share of total weight, because far more fat came off than muscle. And in the one study that measured muscle function rather than mass, function improved and sarcopenic obesity became less common over twelve months. The number that should worry you most is the absolute one, and it matters more the less muscle you had to begin with — which is why the figure looks worse in women over 65 (a quarter to 40% of total loss) than in general trial populations."
      },
      {
        "q": "Why does this site have seven different posts saying different things?",
        "a": "Because seven different studies came out, each answering a different question in a different population, and each was worth reporting when it appeared. Read in sequence they can look contradictory. Read side by side — which is what this page is for — almost all of the disagreement turns out to be a difference in who was measured or how strong the study was, rather than a genuine dispute about the facts."
      },
      {
        "q": "Should I take one?",
        "a": "That is not a question this or any article can answer for you, and anyone writing confidently on the internet about whether you personally should is not in a position to know. What the evidence here can do is tell you which questions to take to the person prescribing: what your starting muscle mass and bone density are, whether a baseline DXA scan makes sense for you, how you will get resistance training and adequate protein in from day one rather than later, and what happens if you stop. That last one matters more than it sounds — a majority of people who start stop within a year."
      },
      {
        "q": "What is the single most useful thing on this page?",
        "a": "That every paper here, however much they appear to disagree, lands on the same two things: resistance training two to three times a week, and more protein than the standard adult guideline — roughly 1.2 to 1.5 g per kilogram of body weight a day for older women, spread across meals. That is the consensus hiding underneath the contradictory headlines."
      }
    ]
  },
  "ua": {
    "cat": "Схуднення",
    "imageAlt": "Великий план: людина робить собі ін’єкцію в живіт за допомогою шприц-ручки.",
    "title": "Препарати GLP-1 після 40: що ми насправді знаємо — і чому заголовки постійно суперечать одне одному",
    "seoTitle": "GLP-1 після 40: що насправді кажуть дослідження",
    "metaDesc": "Сім досліджень GLP-1 поруч. Більшість суперечностей — це різниця в тому, кого вимірювали. Усе в одному місці, з оцінкою надійності кожного висновку.",
    "excerpt": "Одне дослідження каже, що GLP-1 спалюють жир, а не м’язи, інше — що від чверті до 40% втраченого — це м’язи. Вони вимірювали різних жінок різними методами. Тут усе разом.",
    "meta": "21 серпня 2026 · 10 хв читання",
    "lead": "Я писала про препарати GLP-1 сім разів за шість тижнів, і якщо прочитати ці матеріали поспіль, легко вирішити, що в дослідженнях панує хаос. Один каже, що втрачена вага — це переважно жир. Наступний — що від чверті до 40% це м’язи. Третій — що разом із вагою ви втрачаєте кістку, яка вже не повернеться. Здається, ніби вони суперечать одне одному. Майже жодне з них насправді не суперечить — і причина корисніша за будь-який окремий результат.",
    "blocks": [
      {
        "t": "p",
        "text": "Тут діють дві речі, і щойно ви їх побачите, розбіжності здебільшого зникають."
      },
      {
        "t": "p",
        "text": "Перша — кого вимірювали. Дослідження дорослих з ожирінням наприкінці четвертого десятка і огляд жінок віком 65 років і старших описують різні тіла. Той самий препарат може дати заспокійливу цифру в одному й тривожну в іншому, і жодна з них не буде хибною."
      },
      {
        "t": "p",
        "text": "Друга — наскільки надійним було дослідження. Дещо з наведеного нижче — метааналіз десятків рандомізованих досліджень. Дещо — прочитання старіших даних однією дослідницею. Одне з них — доповідь на конференції, яка ще не пройшла рецензування. Вони не заслуговують однакової ваги, а в новинах зазвичай її отримують."
      },
      {
        "t": "h",
        "text": "Усе, що є на цьому сайті про GLP-1, в одній таблиці"
      },
      {
        "t": "p",
        "text": "Прочитайте третю колонку раніше за першу. Саме вона підказує, наскільки можна покладатися на конкретний рядок."
      },
      {
        "t": "table",
        "head": [
          "Що виявили",
          "У кого вимірювали",
          "Наскільки цьому довіряти"
        ],
        "rows": [
          [
            "Більшість втраченої ваги — це жир, а не м’язи",
            "Дорослі з надмірною вагою або ожирінням, із діабетом 2 типу або без — загальні популяції досліджень",
            "Високо — систематичний огляд, що об’єднав 24 рандомізовані дослідження"
          ],
          [
            "Частка м’язової маси в тілі насправді зростає",
            "Дорослі з ожирінням, 7 досліджень, 821 особа",
            "Високо — метааналіз рандомізованих досліджень, проте із дуже високою статистичною неоднорідністю"
          ],
          [
            "Жінки втрачають більше за чоловіків — 10,9% маси тіла проти 6,8%",
            "6 досліджень, 19 906 дорослих — єдині, що звітували в розрізі статі",
            "Помірно — з метааналізу 64 досліджень, але лише 6 із них розділяли за статтю, і довірчі інтервали перекриваються"
          ],
          [
            "Від чверті до 40% втраченого — це м’язова маса",
            "Жінки віком 65 років і старші",
            "Помірно-низько — наративний огляд; дизайни досліджень були надто різними, щоб об’єднати їх статистично"
          ],
          [
            "Додавання ГЗТ до тирзепатиду дало на 35% більше втрати ваги",
            "Жінки в постменопаузі, які вже приймали тирзепатид",
            "Низько — ретроспективна когорта; це зв’язок, а не доказ причини"
          ],
          [
            "Втрата ваги означає втрату кістки, і кістка не повертається",
            "Жінки в постменопаузі; екстраполяція з дослідження 2006 року про фізичні навантаження",
            "Низько — прочитання наявних даних однією експерткою. Жодне дослідження не перевіряло GLP-1 і кістку напряму"
          ],
          [
            "Люди почали робити на 560 кроків на день менше",
            "753 дорослі американці з ожирінням із фітнес-трекерами",
            "Найнижче тут — доповідь на конференції, ще без рецензування"
          ],
          [
            "GLP-1 увійшли до настанов щодо діабету 1 типу",
            "Дорослі з діабетом 1 типу та ІМТ від 30",
            "Консенсус настанов, побудований переважно на дослідженнях ліраглутиду з помірним ефектом"
          ]
        ]
      },
      {
        "t": "h",
        "text": "Скільки ваги насправді йде"
      },
      {
        "t": "p",
        "text": "Найбільший на сьогодні аналіз складу тіла об’єднав 24 рандомізовані дослідження і виявив, що середня маса тіла знизилася приблизно на 9% за три місяці, на 5% за шість і на 4% за дванадцять. Ці спадні цифри виглядають тривожно, доки не помітиш, що вони таке: це окремі групи досліджень, які звітували на кожному з термінів, а не одні й ті самі люди, за якими спостерігали рік. Це не доказ того, що препарати перестають діяти. Здебільшого це відображає те, які дослідження вирішили звітувати коли — і наскільки різнилася стартова вага учасників."
      },
      {
        "t": "p",
        "text": "Друга велика цифра стосується жінок, і тут варто бути точними, бо її подавали дуже неакуратно. Команда Джонса Гопкінса об’єднала 64 рандомізовані дослідження й виявила, що жінки втратили 10,9% маси тіла проти 6,8% у чоловіків. Ви побачите це як «жінки худнуть на 60% більше» — арифметично правильно й шалено перебільшено: в абсолютних величинах різниця становить близько чотирьох відсоткових пунктів маси тіла. Два інші застереження важливіші за заголовок. Лише 6 із тих 64 досліджень узагалі наводили результати окремо за статтю, а довірчі інтервали навколо обох цифр перекриваються (7,0–14,8% у жінок, 4,6–9,0% у чоловіків). І акцент самих дослідників був майже протилежним до того, як це розійшлося: їхній головний висновок — що ці препарати діють приблизно однаково незалежно від віку, раси, етнічності, стартового ІМТ чи стартового HbA1c. Стать була єдиним винятком у всьому аналізі. [Повний розбір тут](/ua/blog/glp1-weight-loss-sex-difference-women-men/)."
      },
      {
        "t": "p",
        "text": "Один виняток має значення, якщо ви обираєте між препаратами: цей аналіз свідомо не включав тирзепатид (Mounjaro, Zepbound), бо він діє ще й на другий кишковий гормон. Жодна з цих цифр його не описує."
      },
      {
        "t": "h",
        "text": "Питання м’язів — саме тут і живе справжня плутанина"
      },
      {
        "t": "p",
        "text": "Саме воно породжує суперечливі заголовки, і воно має чітке розв’язання, щойно помічаєш, що дослідження відповідають на три різні запитання."
      },
      {
        "t": "p",
        "text": "Чи втрачаєте ви м’язи в абсолютних величинах? Так. Метааналіз семи рандомізованих досліджень за участю 821 особи виявив, що абсолютна м’язова маса знизилася в середньому приблизно на 1,74 кг, а на семаглутиді — значно більше. У жінок віком 65 років і старших [огляд оцінив частку м’язової маси в загальній втраті ваги приблизно у чверть-40%](/ua/blog/glp1-older-women-lean-mass/). У дослідженні SEMALEAN — 106 пацієнтів, переважно жінки — абсолютна м’язова маса впала приблизно на 3 кг у перші місяці."
      },
      {
        "t": "p",
        "text": "Чи падає частка м’язів у тілі? Ні — вона зростає. Той самий метааналіз семи досліджень виявив, що м’язова маса як частка загальної маси тіла зросла на 1,81%. Це не суперечить попередньому абзацу: так і буває, коли одночасно втрачаєш багато жиру й трохи м’язів. У вас лишається менше кілограмів м’язів на тілі, яке при цьому складається з м’язів на більший відсоток, ніж раніше."
      },
      {
        "t": "p",
        "text": "Чи гірше працюють ваші м’язи? У єдиному дослідженні, яке це перевіряло, — ні. У SEMALEAN частка учасників із саркопенічним ожирінням (забагато жиру й замало м’язів водночас) знизилася з 49% на початку до 33% через дванадцять місяців. Раннє зниження м’язової маси стабілізувалося протягом спостереження, а показники функції м’язів покращилися. Автори дійшли висновку, що сама лише втрата м’язової маси не є підставою відмовляти в цих препаратах."
      },
      {
        "t": "p",
        "text": "Отже, [«переважно жир, а не м’язи»](/ua/blog/glp1-weight-loss-drugs-fat-not-muscle/) і «чверть-40% це м’язи» — той самий висновок, прочитаний з протилежних боків. Чверть-40% усе одно лишає більшість втраченого на жир. Який бік важливіший саме для вас, залежить майже цілком від того, скільки м’язів у вас було на старті — і це чесна причина, чому заспокійлива версія походить із загальних дорослих популяцій, а тривожна — від жінок віком 65 років і старших."
      },
      {
        "t": "h",
        "text": "Кістки — їм приділяють частку уваги, а доказів найменше"
      },
      {
        "t": "p",
        "text": "Венді Корт вивчає кістки й фізичні навантаження 35 років, і її позиція пряма: втрата ваги означає втрату кісткової маси. Її власне дослідження 2006 року — найнезручніша частина. Жінки на шестимісячній програмі тренувань під наглядом втратили близько десяти фунтів, і, що незвично, втрата припала на жир, а не на м’язи — і вони все одно втратили більше кісткової маси, ніж жінки, які не худнули. За рік подальшого спостереження щільність кісток не відновилася ані в тих, хто набрав вагу назад, ані в тих, хто не набрав."
      },
      {
        "t": "p",
        "text": "Якщо додати, що жінки віком 50–64 роки — найактивніші користувачки цих препаратів, і що до двох третин тих, хто починає, припиняють протягом року й повертають більшу частину ваги, її занепокоєння таке: багато жінок проходять повторні цикли схуднення й набору, які щоразу віднімають кістку, не додаючи її назад. Її практична порада — зробити базове DXA-сканування до початку. [Повний аргумент і його межі тут](/ua/blog/glp1-menopause-bone-loss/)."
      },
      {
        "t": "p",
        "text": "Але варто чітко розуміти, що це таке: прочитання наявних даних експерткою, а не дослідження. Жодне дослідження не перевіряло препарати GLP-1 і щільність кісток напряму в цій групі, а оглядове дослідження, опубліковане в січні, дійшло висновку, що потрібні більші дослідження, перш ніж щось стверджувати. Це найслабший доказ на цій сторінці й водночас найважливіше питання на ній — незручне поєднання, а не привід відмахнутися."
      },
      {
        "t": "h",
        "text": "Якщо ви в перименопаузі або після неї"
      },
      {
        "t": "p",
        "text": "Два висновки стосуються цього напряму, і обидва — із застереженням. Жінки в постменопаузі, які приймали гормональну терапію разом із тирзепатидом, [втратили приблизно на 35% більше ваги](/ua/blog/tirzepatide-hrt-weight-loss-menopause-2026/), ніж ті, хто приймав лише тирзепатид: −19,2% проти −14,0% за дванадцять місяців. Це вражаюча цифра з ретроспективного когортного дослідження, тобто вона показує зв’язок і не може виключити, що жінки, які обирають гормональну терапію, відрізняються ще чимось, що теж допомагає з вагою."
      },
      {
        "t": "p",
        "text": "Друге — радше гіпотеза, ніж висновок. Серед пояснень статевої різниці автори з Джонса Гопкінса називали можливу синергію з естрогеном. Якщо річ саме в цьому, постає питання, на яке ніхто не відповів: чи зберігається жіноча перевага після падіння естрогену в менопаузі? Даних немає в жодному напрямку — і варто знати, що прогалина в доказах лежить рівно там, де стоїть більшість читачок цього сайту."
      },
      {
        "t": "p",
        "text": "Крізь призму Корт більша втрата ваги — теж не однозначно добра новина для скелета, хоча сам естроген захищає кістки, що тягне в інший бік. Обидва твердження правдиві, і ніхто не провів дослідження, яке б це вирішило."
      },
      {
        "t": "h",
        "text": "А якщо у вас діабет 1 типу"
      },
      {
        "t": "p",
        "text": "Настанови ADA 2026 вперше підтримують терапію на основі GLP-1 для лікування ожиріння в дорослих із діабетом 1 типу та ІМТ від 30. Це рекомендація щодо ваги, яка накладається на інсулін, а не замінює його, і препарати досі не ліцензовані спеціально для діабету 1 типу. Доказова база — переважно ліраглутид із помірними середніми ефектами, а настанови окремо наголошують на реальному ризику кетоацидозу навіть тоді, коли глюкоза виглядає майже нормальною. [Що змінилося і що це означає для жінок](/ua/blog/ada-2026-glp1-type1-diabetes-women/)."
      },
      {
        "t": "h",
        "text": "Єдине, з чим погоджується кожна робота"
      },
      {
        "t": "p",
        "text": "Це та частина, яку варто лишити, навіть якщо забудете все інше. Метааналіз складу тіла, огляд щодо старших жінок, дослідниця кісток і настанови щодо діабету 1 типу — усі приходять до тих самих двох важелів: силові тренування й достатньо білка. Не кардіо, не доза препарату, не добавка. Огляд щодо жінок після 65 найконкретніший: силова робота двічі-тричі на тиждень загалом близько 150 хвилин і 1,2–1,5 г білка на кілограм маси тіла на добу, рівномірно розподілені між прийомами їжі, а не зібрані в один. Обидві цифри вищі за стандартні норми для дорослих, і це рекомендації авторів на основі ширшої літератури, а не результат дослідження, яке порівнювало б цей комбінований протокол із самим лише препаратом."
      },
      {
        "t": "p",
        "text": "Через що ще один висновок стає незручним. Відстежуючи дані носимих пристроїв 753 дорослих, дослідники виявили, що після початку прийому GLP-1 середня кількість кроків за день знизилася приблизно на 560, а час помірно-інтенсивних навантажень — приблизно з 28 до 22 хвилин. Це найслабше дослідження тут — доповідь на конференції без рецензування — і воно описує середнє, а не правило. Але воно вказує в неправильний бік саме тоді, коли докази кажуть, що рух важить найбільше. [Деталі тут](/ua/blog/glp1-drugs-less-movement/). Препарат знижує апетит; він не будує силу, і ніщо в цій літературі не свідчить, що тренування з’являться самі."
      },
      {
        "t": "h",
        "text": "Як читати наступний заголовок про GLP-1"
      },
      {
        "t": "p",
        "text": "Три запитання, саме в такому порядку, — і вони зроблять більшу частину роботи."
      },
      {
        "t": "p",
        "text": "Кого вимірювали? Популяції досліджень зміщені до середнього віку. Якщо висновок стосується жінок після 65 або саме жінок у постменопаузі, він не збігатиметься із загальними цифрами — а якщо в заголовку не сказано, припускайте, що йдеться про загальну популяцію."
      },
      {
        "t": "p",
        "text": "Який це був тип дослідження? Метааналіз рандомізованих досліджень, наративний огляд, спостережна когорта й тези конференції — це чотири дуже різні речі, які носять одне слово «дослідження»."
      },
      {
        "t": "p",
        "text": "Відсоток від чого? «Жінки втрачають на 60% більше» і «жінки втрачають на чотири відсоткові пункти більше» описують той самий результат. Так само як «більшість — це жир» і «третина — це м’язи»."
      },
      {
        "t": "p",
        "text": "Ніщо з цього не є аргументом за чи проти прийому. Це аргумент за те, щоб розуміти, на яке запитання відповідає конкретна цифра, — і сприймати два важелі, з якими погоджуються всі, як частину лікування, а не як необов’язковий додаток."
      },
      {
        "t": "h",
        "text": "Що каже наука"
      },
      {
        "t": "refs",
        "items": [
          {
            "claim": "У 24 об’єднаних рандомізованих дослідженнях більшість втраченої на препаратах GLP-1 ваги припадала на жир, а не на м’язову тканину.",
            "detail": "Систематичний огляд і метааналіз в International Journal of Obesity: пошук до грудня 2024 року, 36 досліджень у якісному аналізі, 24 об’єднані статистично, підгрупи за типом препарату й тривалістю лікування. Середня маса тіла знизилася приблизно на 9% за три місяці, 5% за шість і 4% за дванадцять. Переважало зменшення жирової маси, зменшення м’язової було помірним. Результатів у розрізі статі не наводить.",
            "cite": "International Journal of Obesity · Систематичний огляд і метааналіз, 25 квітня 2026 (PMID 42034831)",
            "url": "https://pubmed.ncbi.nlm.nih.gov/42034831/"
          },
          {
            "claim": "М’язова маса зросла як частка маси тіла (+1,81%), водночас знизившись в абсолютних величинах (−1,74 кг).",
            "detail": "Метааналіз семи рандомізованих контрольованих досліджень за участю 821 пацієнта з ожирінням, які отримували препарати в дозах для лікування ожиріння, проти плацебо. М’язова маса як частка загальної маси зросла на 1,81% (95% ДІ 1,1–2,52). Абсолютна м’язова маса знизилася на 1,74 кг (95% ДІ −3,04 до −0,45), відсоток м’язової маси — на 3,06%, обидва з дуже високою неоднорідністю (I²=98%); найбільше абсолютне зниження показав семаглутид — −5,44 кг. Автори роблять висновок, що втрата м’язової маси не має вважатися обмеженням для застосування цих препаратів за умови поєднання з харчуванням і фізичними навантаженнями.",
            "cite": "International Journal of Obesity · Систематичний огляд і метааналіз, 19 червня 2026 (PMID 42321502)",
            "url": "https://pubmed.ncbi.nlm.nih.gov/42321502/"
          },
          {
            "claim": "Жінки втратили 10,9% маси тіла проти 6,8% у чоловіків — із перекриттям довірчих інтервалів, за даними 6 із 64 досліджень.",
            "detail": "Систематичний огляд і метааналіз команди Джонса Гопкінса в JAMA Internal Medicine: пошук до 26 липня 2024 року, 41 стаття, що представляє 64 рандомізовані дослідження. Серед 6 досліджень (19 906 пацієнтів), проаналізованих за статтю, втрата ваги була більшою в жінок (10,9%; 95% ДІ 7,0–14,8), ніж у чоловіків (6,8%; 95% ДІ 4,6–9,0). Лише 10 із 48 окремо охарактеризованих досліджень узагалі оцінювали стать. Значущої неоднорідності ефекту за віком, расою, етнічністю, стартовим ІМТ чи стартовим HbA1c не виявлено. Тирзепатид не включали.",
            "cite": "JAMA Internal Medicine · Систематичний огляд і метааналіз, 2026 (PMID 41770554)",
            "url": "https://pubmed.ncbi.nlm.nih.gov/41770554/"
          },
          {
            "claim": "У жінок віком 65 років і старших приблизно 25–40% загальної втрати ваги припадало на м’язову масу.",
            "detail": "Наративний огляд у Nutrients, що узагальнює дані досліджень 3-ї фази та реальної практики щодо старших жінок. Жінки віком ≥65 досягали стійкої втрати ваги 10–20%. Приблизно 25–40% загальної втрати ваги віднесено до зменшення м’язової маси. У дослідженні SEMALEAN (106 пацієнтів, переважно жінки) абсолютна м’язова маса знизилася приблизно на 3 кг на ранньому етапі, потім стабілізувалася, функція м’язів покращилася, а поширеність саркопенічного ожиріння впала з 49% на початку до 33% через 12 місяців. Огляд рекомендує силові навантаження 2–3 рази на тиждень (близько 150 хвилин) і 1,2–1,5 г білка на кг маси тіла на добу, розподілені між прийомами їжі. Дані узагальнено наративно, бо дизайни досліджень були надто неоднорідними для об’єднання.",
            "cite": "Nutrients · Наративний огляд, 14 лютого 2026 (PMID 41754149)",
            "url": "https://pubmed.ncbi.nlm.nih.gov/41754149/"
          },
          {
            "claim": "Втрата ваги спричиняє втрату кісткової маси навіть тоді, коли втрачається жир, — і кістка не повертається разом із вагою.",
            "detail": "Венді Корт (Університет Колорадо, Аншутц) описує своє дослідження 2006 року з контрольованими фізичними навантаженнями та річне спостереження після нього, у якому щільність кісток не відновилася ані в учасниць, які набрали вагу назад, ані в тих, хто не набрав. Вона повідомляє, що близько 15% дорослих американок приймали препарат GLP-1, найбільше — у віці 50–64 років. Це експертне інтерв’ю з прочитанням наявних даних, а не дослідження GLP-1 і кісток.",
            "cite": "UCHealth Today · Експертне інтерв’ю, 22 червня 2026",
            "url": "https://www.uchealth.org/today/menopause-and-glp-1-weight-loss-drugs-and-bone-loss/"
          },
          {
            "claim": "Додавання гормональної терапії до тирзепатиду пов’язане з на 35% більшою втратою ваги після менопаузи.",
            "detail": "Ретроспективне когортне дослідження жінок у постменопаузі на тирзепатиді, яке порівнювало тих, хто додатково застосовував менопаузальну гормональну терапію, з тими, хто ні. Загальне зниження маси тіла становило −19,2% проти −14,0% за 12 місяців (p=0,0023). Спостережне: показує зв’язок і не може встановити причину.",
            "cite": "The Lancet Obstetrics, Gynaecology & Women’s Health · Ретроспективне когортне дослідження, 2026",
            "url": "https://www.thelancet.com/journals/lanogw/article/PIIS3050-5038(25)00145-1/abstract"
          },
          {
            "claim": "Настанови ADA 2026 підтримують терапію на основі GLP-1 для лікування ожиріння в дорослих із діабетом 1 типу.",
            "detail": "Уперше терапію на основі агоністів рецепторів GLP-1 включено як опцію для лікування ожиріння в дорослих із діабетом 1 типу та ІМТ від 30 (27,5 для американців азійського походження). Це експертний консенсус на основі об’єднаних даних; найбільші дослідження при діабеті 1 типу використовували ліраглутид 1,8 мг із помірними середніми ефектами, а настанови наголошують на ризиках гіпоглікемії та еуглікемічного кетоацидозу.",
            "cite": "American Diabetes Association · Standards of Care in Diabetes—2026, розділ 8",
            "url": "https://diabetesjournals.org/care/article/49/Supplement_1/S166/163915/8-Obesity-and-Weight-Management-for-the-Prevention"
          },
          {
            "claim": "Після початку прийому GLP-1 кількість кроків за день знизилася приблизно на 560, а помірно-інтенсивні навантаження — з 28 до 22 хвилин.",
            "detail": "Дані фітнес-трекерів 753 дорослих з ожирінням у програмі NIH All of Us, порівняння активності до й після початку прийому GLP-1. Найбільше зниження — у чоловіків і в людей із скелетно-м’язовим болем. Представлено на конференції, ще без рецензування.",
            "cite": "Endocrine Society (ENDO 2026) · Доповідь на конференції",
            "url": "https://www.healio.com/news/endocrinology/20260613/exercise-declined-in-patients-with-obesity-after-starting-a-glp1"
          }
        ]
      },
      {
        "t": "links",
        "title": "Усі матеріали про GLP-1 на цьому сайті",
        "items": [
          {
            "name": "Втрата ваги означає втрату кісткової маси — аргументи на користь обережності з GLP-1 у менопаузі",
            "url": "/ua/blog/glp1-menopause-bone-loss/",
            "desc": "Питання кісток і чому доказів для нього менше, ніж заслуговує саме занепокоєння."
          },
          {
            "name": "Препарати GLP-1 у жінок після 65 — яка частка втрати це м’язи",
            "url": "/ua/blog/glp1-older-women-lean-mass/",
            "desc": "Звідки взялася цифра «чверть-40%» і який протокол пропонують автори замість самих лише пересторог."
          },
          {
            "name": "Жінки втрачають на препаратах GLP-1 більше ваги, ніж чоловіки",
            "url": "/ua/blog/glp1-weight-loss-sex-difference-women-men/",
            "desc": "64 дослідження — і справжній акцент авторів був на тому, як мало що інше мало значення."
          },
          {
            "name": "Більшість ваги, втраченої на GLP-1, — це жир, а не м’язи",
            "url": "/ua/blog/glp1-weight-loss-drugs-fat-not-muscle/",
            "desc": "Заспокійлива цифра й загальні дорослі популяції, у яких її виміряли."
          },
          {
            "name": "Гормональна терапія плюс тирзепатид: на 35% більше втрати ваги після менопаузи",
            "url": "/ua/blog/tirzepatide-hrt-weight-loss-menopause-2026/",
            "desc": "Вражаючий результат зі спостережного дослідження — з усіма застереженнями, які це тягне."
          },
          {
            "name": "Настанови з діабету 2026 підтримують препарати GLP-1 при діабеті 1 типу",
            "url": "/ua/blog/ada-2026-glp1-type1-diabetes-women/",
            "desc": "Що змінилося в настановах і чому для жінок із діабетом 1 типу це звучить інакше."
          },
          {
            "name": "Люди на препаратах GLP-1 почали рухатися менше, а не більше",
            "url": "/ua/blog/glp1-drugs-less-movement/",
            "desc": "Висновок із носимих пристроїв, який ускладнює будь-яку пораду «просто додайте силові»."
          },
          {
            "name": "Більше білка, ніж рекомендує офіційна норма, підтримує м’язи й кістки після менопаузи",
            "url": "/ua/blog/higher-protein-postmenopausal-bone-muscle/",
            "desc": "Білкова половина поради, до якої сходиться кожна з цих робіт."
          },
          {
            "name": "Як почати силові тренування в перименопаузі",
            "url": "/ua/blog/strength-training-perimenopause-beginners/",
            "desc": "Тренувальна половина — з нуля."
          }
        ]
      }
    ],
    "faq": [
      {
        "q": "То препарати GLP-1 призводять до втрати м’язів чи ні?",
        "a": "Обидві відповіді є в доказах, бо вони вимірюють різні речі. В абсолютних величинах — так: метааналіз семи досліджень виявив зниження м’язової маси в середньому приблизно на 1,74 кг. Як частка тіла — ні: той самий аналіз показав зростання частки м’язової маси на 1,81% від загальної маси, бо жиру пішло значно більше, ніж м’язів. А в єдиному дослідженні, яке вимірювало функцію м’язів, а не масу, функція покращилася, а саркопенічне ожиріння стало рідшим за дванадцять місяців. Найбільше має турбувати саме абсолютна цифра — і вона важить тим більше, чим менше м’язів у вас було на старті. Саме тому показник виглядає гірше в жінок після 65 (чверть-40% загальної втрати), ніж у загальних популяціях досліджень."
      },
      {
        "q": "Чому на цьому сайті сім різних матеріалів, які кажуть різне?",
        "a": "Бо вийшло сім різних досліджень, кожне відповідало на своє запитання у своїй популяції, і кожне варто було висвітлити тоді, коли воно з’явилося. Прочитані поспіль, вони можуть здаватися суперечливими. Прочитані поруч — а саме для цього ця сторінка — майже всі розбіжності виявляються різницею в тому, кого вимірювали або наскільки надійним було дослідження, а не справжньою суперечкою про факти."
      },
      {
        "q": "Чи варто мені їх приймати?",
        "a": "На це запитання не може відповісти ані ця, ані будь-яка інша стаття, і той, хто впевнено пише в інтернеті, чи варто це особисто вам, не має змоги це знати. Що можуть дати ці докази — це перелік запитань до лікаря, який призначає: яка у вас стартова м’язова маса й щільність кісток, чи має сенс базове DXA-сканування, як ви забезпечите силові тренування й достатньо білка з першого дня, а не потім, і що буде, якщо ви припините. Останнє важливіше, ніж звучить: більшість тих, хто починає, припиняють протягом року."
      },
      {
        "q": "Що найкорисніше на цій сторінці?",
        "a": "Те, що кожна робота тут, хоч якими різними вони здаються, приходить до тих самих двох речей: силові тренування двічі-тричі на тиждень і більше білка, ніж стандартна норма для дорослих, — приблизно 1,2–1,5 г на кілограм маси тіла на добу для старших жінок, розподілені між прийомами їжі. Це і є консенсус, схований під суперечливими заголовками."
      }
    ]
  }
};
