// Same date as 1 earlier post; seq keeps the hub order stable.
export const seq = 1;

export default {
  "slug": "night-sweats-or-hypo",
  "date": "2026-08-19",
  "image": "/images/night-sweats-or-hypo.jpg",
  "cats": [
    "diabetes",
    "perimenopause"
  ],
  "en": {
    "cat": "Diabetes & Perimenopause",
    "imageAlt": "A woman lying awake in bed at night, one hand resting on her forehead.",
    "title": "Night sweats or a hypo? How to tell them apart when you have both perimenopause and diabetes",
    "seoTitle": "Night Sweats or a Hypo? How to Tell the Difference",
    "metaDesc": "Perimenopause and nocturnal hypoglycaemia cause near-identical night sweats. How to tell them apart safely — from 23 years with type 1 diabetes.",
    "excerpt": "Perimenopause and a night-time hypo can bring almost the same 3am wake-up, and you can’t tell them apart by feel. Here’s what settles it, and why the order matters.",
    "meta": "19 August 2026 · 9 min read",
    "lead": "Menopause websites will tell you night sweats are a vasomotor symptom. Diabetes websites will tell you they are a warning sign of a low overnight. Both are written for a woman who belongs to one category, and if you have diabetes and you are somewhere in perimenopause, you don’t. This is the post for the middle.",
    "blocks": [
      {
        "t": "quote",
        "text": "This article is education, not medical advice, and it is deliberately not a symptom checklist. If you wake in the night with anything that could be a low blood sugar, check your glucose first and interpret afterwards — a reading takes seconds, and nothing written on this page, or any page, can stand in for it. There is no dosing advice here. How you treat a low, and any change to insulin or medication, is a conversation for your own diabetes team."
      },
      {
        "t": "h",
        "text": "The 3am problem"
      },
      {
        "t": "p",
        "text": "You wake at three in the morning and the sheets are wet through. Your heart is going. You are hot, then cold, then shaking very slightly, and there is a particular kind of dread attached to it that never quite survives being described in daylight. By the time you have got up, changed your top and lain back down, you have already told yourself something — it’s the menopause, or it was a hypo, or it’s just stress."
      },
      {
        "t": "p",
        "text": "That sentence, the one you say to yourself before you have checked anything, is the whole problem. It is almost never based on this episode. It is based on the last one, and on which of your two conditions you have been mentally blaming lately."
      },
      {
        "t": "p",
        "text": "There is a reason “it’s probably the menopause” should make you uncomfortable as a default. It is an answer that costs nothing to reach and, on the night it is wrong, costs a great deal. So this piece does not open with a list of the ways the two feel different. It opens by saying, as plainly as I can, that they largely don’t."
      },
      {
        "t": "h",
        "text": "Why they feel so similar"
      },
      {
        "t": "p",
        "text": "When blood glucose starts falling, the body mounts a counter-regulatory response, and the most noticeable part of it is adrenaline. Adrenaline produces sweating, a pounding or racing heart, tremor, a wash of anxiety, and — sometimes — waking up. That cluster is not a signature of low glucose. It is a signature of adrenaline, which is a different thing entirely."
      },
      {
        "t": "p",
        "text": "A hot flush arrives from another direction and recruits overlapping machinery. As oestrogen falls and fluctuates, the hypothalamic thermoneutral zone — the band of core temperature the body is content to sit in — narrows. A small rise that would once have passed unnoticed now breaches the ceiling, and the body dumps heat: blood vessels dilate, you flush, you sweat, and then you get cold as you overshoot in the other direction. Palpitations and abrupt arousal come along with it often enough that women describe the whole thing in the same words they would use for a low."
      },
      {
        "t": "p",
        "text": "So what you feel at 3am is your body’s response to something, not the something itself, and two different triggers are pulling on the same alarm. This is why the sensation cannot tell you the cause. Breakthrough T1D’s own menopause guidance puts it about as directly as a patient resource ever does: hot sweats and palpitations feel similar to hypoglycaemia, and where the two overlap you always treat the low first."
      },
      {
        "t": "h",
        "text": "What a nocturnal hypo actually is"
      },
      {
        "t": "p",
        "text": "Glucose falling below your target range while you are asleep — driven by long-acting insulin, by exercise earlier in the day, by alcohol, by an evening meal that was smaller or later than usual, or by insulin sensitivity that has quietly shifted. Perimenopause belongs on that list too, because changing oestrogen and progesterone move insulin sensitivity around, which is the subject of [the pillar piece on what happens to blood sugar as oestrogen falls](/blog/perimenopause-diabetes-blood-sugar/)."
      },
      {
        "t": "p",
        "text": "The uncomfortable part is what happens next. A controlled laboratory study published in PLoS Medicine in 2007 lowered glucose overnight in 16 people with type 1 diabetes and 16 matched healthy controls, under full sleep monitoring. Ten of the sixteen controls woke up. One of the sixteen people with type 1 diabetes did. Not one in three. One."
      },
      {
        "t": "p",
        "text": "Which means “I would have woken up if I had gone low” is not a safe premise, and neither is its mirror image — that because you did wake up, it must have been a low. Waking is a poor detector in both directions."
      },
      {
        "t": "p",
        "text": "There is a morning signature worth knowing: a headache on waking, sleep that did not refresh, flat mood and unusual fatigue the following day. It is suggestive rather than diagnostic. And in a 2023 study in Scientific Reports that paired sensor data with nightly sleep ratings in 27 adults with type 1 diabetes, the relationship ran in a direction most people find surprising — it was the frequency and duration of the milder lows that tracked with poor sleep, while the nights that went below 3.0 mmol/L — when the low typically lasted around two and a half hours in total — made no measurable difference to how people rated the night at all. How bad it felt and how bad it was are not the same axis."
      },
      {
        "t": "h",
        "text": "What a perimenopausal night sweat actually is"
      },
      {
        "t": "p",
        "text": "The same thermoregulatory story as a daytime flush, with the disadvantage that you were unconscious when it started, so you arrive in the middle of it with no run-up to interpret. Vasomotor symptoms reach 60–80% of women at some point in the transition, peaking in late perimenopause and the first years after the final period — and they are associated with every dimension of disturbed sleep there is: falling asleep, staying asleep, and waking too early."
      },
      {
        "t": "p",
        "text": "That last part matters for anyone with diabetes, because broken sleep is not a neutral outcome. It pushes insulin resistance up the next day, which is its own loop, and which I have written about separately in [the piece on sleep and insulin resistance](/blog/sleep-insulin-resistance-women/)."
      },
      {
        "t": "h",
        "text": "Why comparing symptoms is a losing strategy"
      },
      {
        "t": "p",
        "text": "It is worth being clear about why this post does not give you a comparison table, because “there are no differences” would be too strong and is not what I mean. Tendencies do exist. A flush more often begins as a wave of heat in the chest and face that travels upward and passes in a few minutes. A low more often brings hunger and tremor, and settles as the number comes up rather than on its own. The problem is not that nothing distinguishes them. It is that those are tendencies across a population, and you are applying them to one episode, in the dark, once."
      },
      {
        "t": "p",
        "text": "And the tendencies sit on top of a shared response. Sweating, a racing heart and abrupt anxious waking are what a body does when it is alarmed. They do not arrive with a label saying which alarm was pulled. Any list of them describes the bell, not the hand on the rope."
      },
      {
        "t": "p",
        "text": "Then there is the awkward fact that the warning symptoms are not equally available to everyone. Impaired awareness of hypoglycaemia — where the usual early warnings are muted, or missing altogether — was found in 17.5% of adults with type 1 diabetes in a 2024 questionnaire study, and other estimates run up to around 30%. What matters is what it travels with: in that study, 37.5% of people with impaired awareness had had a severe hypo in the previous year, against 11.7% of those with normal awareness — roughly three times the rate, and four and a half times the odds. The symptoms are least informative exactly where the consequences are worst. A checklist assumes a working alarm, and a meaningful minority of the people reading this do not have one."
      },
      {
        "t": "p",
        "text": "Consider, too, when the judgement is being made. Half asleep, in the dark, at the hour your reasoning is poorest anyway — and if it is a low, the faculty you are using to assess it is being degraded by the very thing you are assessing. Falling glucose impairs judgement before it announces that it has. You are asking the affected instrument to measure the effect."
      },
      {
        "t": "p",
        "text": "The most corrosive part, though, is that guessing teaches you nothing. If you decide it was a flush and go back to sleep, nothing ever contradicts you. Morning comes, the episode is over, and the decision quietly files itself as correct because nothing arrived to say otherwise. Do that for a year and your confidence grows while your accuracy stays exactly where it was — and that is not a state you can detect from the inside. I know, because I was in it. The only thing that generates real feedback is a number."
      },
      {
        "t": "p",
        "text": "And the two mistakes do not cost the same. Treating a flush as though it were a low costs you some glucose you did not need and a bit of irritation with yourself. Treating a low as though it were a flush and going back to sleep has a floor you do not want to find. When the errors are that lopsided, the goal stops being a better guess. It becomes not needing one."
      },
      {
        "t": "h",
        "text": "The one thing that reliably separates them"
      },
      {
        "t": "p",
        "text": "A number."
      },
      {
        "t": "p",
        "text": "That is the whole answer, and I would rather give you the useful version than a satisfying one. At the moment it is happening, a glucose reading is the only thing that tells you which of the two you are in. Twenty-three years of practice does not substitute for it. Neither does the presence or absence of any particular sensation."
      },
      {
        "t": "p",
        "text": "So the practical change is embarrassingly small: put the meter within arm’s reach of the bed. The reason people don’t check at 3am is almost never scepticism, it is that the kit is downstairs and they are half asleep and it is easier to decide than to walk. Check, then decide — in that order, including on the nights you are certain."
      },
      {
        "t": "p",
        "text": "If the number is low, treat it the way you and your team have already agreed, and check again afterwards to confirm it has come up. If the number is in range, you have learned something solid: that episode was not a low. Not “probably not”. Was not — and that is a fact you can take to an appointment."
      },
      {
        "t": "p",
        "text": "One caveat if you are reading a sensor rather than a fingerstick. Sensors measure interstitial fluid and lag behind blood by several minutes, and they are least accurate exactly when glucose is moving fast. That error is not symmetrical in its consequences: a sensor showing you in range during a rapid fall is the reading you least want to trust. If the symptoms and the number disagree, confirm with a fingerstick."
      },
      {
        "t": "h",
        "text": "If it is both at once"
      },
      {
        "t": "p",
        "text": "It will be, some nights. The two conditions do not take turns, and there is no reason a flush cannot land on a night your glucose is also drifting down. The guidance for that is unambiguous and worth stating on its own line: treat the low first. A hot flush is miserable and it is not dangerous. An untreated low is the other way round. The order of operations is not a close call."
      },
      {
        "t": "h",
        "text": "What a CGM changes about this question"
      },
      {
        "t": "p",
        "text": "A fingerstick gives you one point. A sensor gives you the shape of the night — whether you had been sliding since midnight, whether you dipped and rebounded, or whether the line was flat the whole time your body was busy doing something else."
      },
      {
        "t": "p",
        "text": "Two things follow. The first is alarms: an overnight low alert is the direct, practical answer to that PLoS Medicine finding. If your own awakening response is unreliable, you borrow one that isn’t. Set it, and then tolerate it being slightly more irritating than you would like."
      },
      {
        "t": "p",
        "text": "The second is the part that actually resolves the question, and it is retrospective. Take two to three weeks of overnight traces and mark the nights you woke. If the episodes cluster with lows, you have your answer. If they land on flat, in-range nights, you have a different one. If it is mixed — and for a lot of women in perimenopause it is — then you know that too, and knowing it is more useful than either clean story, because it tells you that last night’s explanation cannot be reused tonight."
      },
      {
        "t": "p",
        "text": "Be honest about what a sensor is, though. The lag and the rapid-change error are real, and so are compression lows from sleeping on the sensor. It is an excellent pattern tool across weeks and a mediocre oracle in a single moment. If you are weighing up which one to use, I keep [a side-by-side comparison of the CGMs available here](/cgm-comparison/) up to date, and [the blood sugar investigator](/blood-sugar-investigator/) walks through overnight patterns question by question if you would rather be guided than read a table."
      },
      {
        "t": "h",
        "text": "Reading your overnight traces for patterns"
      },
      {
        "t": "table",
        "head": [
          "What you see overnight",
          "What it tends to point to"
        ],
        "rows": [
          [
            "A gentle, steady decline after midnight, dipping low before about 4am",
            "The commonest shape of a nocturnal low. A cluster analysis of 2,519 overnight traces from 395 adults with type 1 diabetes found most hypoglycaemic episodes falling in that midnight-to-4am window. It is a basal or evening-dose question for your team — not something to adjust off the back of an article."
          ],
          [
            "A dip in the small hours followed by a climb, so you wake at 3am and find a high number at breakfast",
            "Post-hypoglycaemic nocturnal hyperglycaemia — either the counter-regulatory response overshooting or the treatment for the low doing so; the study could not tell which. In a 2025 analysis of 755 adults with type 1 diabetes wearing sensors, around a third had at least one such episode in a fortnight. The morning high is the clue that something happened earlier, and treating the high without looking at what preceded it is exactly the trap."
          ],
          [
            "A flat, in-range line across the whole night, on a night you definitely woke",
            "That episode was not a low. If it repeats on in-range nights, the conversation moves to vasomotor symptoms and belongs with your GP or menopause clinician."
          ],
          [
            "Lows on the nights after harder exercise, or after a drink, or after a smaller evening meal",
            "A pattern with a modifiable trigger attached. Alcohol in particular can drop glucose many hours after the drink, which is a large enough subject to deserve its own post. Bring the pairing to your team rather than solving it alone."
          ],
          [
            "Nothing consistent — some episodes with lows, some without",
            "The most common real-world answer, and not a failure of the exercise. It means you cannot use the last episode to interpret this one, which is precisely the reason to keep checking."
          ]
        ]
      },
      {
        "t": "h",
        "text": "What to bring to your appointment"
      },
      {
        "t": "p",
        "text": "None of this is self-diagnosis. It is evidence-gathering, and the difference matters, because the thing that changes a clinician’s mind is not a better description of how the episodes feel — it is data they can read in thirty seconds."
      },
      {
        "t": "table",
        "head": [
          "What to bring",
          "Why it helps"
        ],
        "rows": [
          [
            "Dates and times of the episodes, even approximate",
            "Turns “I keep waking up sweating” into a frequency and a position on the clock, which is what makes it a clinical question rather than a complaint."
          ],
          [
            "The glucose reading at each episode — and whether you got one at all",
            "The single most valuable column. An episode with a documented in-range number is evidence in a way that a described sensation never is."
          ],
          [
            "Whether treating changed anything",
            "If you treated a low and the symptoms settled as the number came up, that supports one explanation. If you treated and felt no different, say so — that is informative in the opposite direction."
          ],
          [
            "Screenshots of your overnight traces for the last two to three weeks",
            "Shows the pattern rather than the anecdote. A handful of representative nights beats a summary you have written yourself."
          ],
          [
            "The context around each episode: exercise, alcohol, evening meal timing, illness, where you are in your cycle if you still have one",
            "These get asked about anyway, and half-remembered answers burn most of a ten-minute appointment."
          ],
          [
            "Whether you are on HRT, or thinking about it",
            "It changes both conversations at once. HRT can affect insulin sensitivity and glucose behaviour, so the diabetes team needs to know about it even though someone else prescribed it."
          ]
        ]
      },
      {
        "t": "p",
        "text": "If HRT is on the table for you, the evidence sits in [the piece on HRT and type 1 diabetes](/blog/hrt-type-1-diabetes-evidence/) — worth reading before either appointment rather than after."
      },
      {
        "t": "p",
        "text": "And then the part nobody warns you about: your diabetes team and your GP or menopause clinician are almost certainly not talking to each other. Not out of negligence — different services, different records, different appointment cycles, and a referral letter that says nothing about night sweats. In practice you are the only person who attends both conversations, which makes you the bridge whether you wanted the job or not. Take the same document to each of them, and ask each one directly what they would want the other to know."
      },
      {
        "t": "h",
        "text": "What I do now"
      },
      {
        "t": "p",
        "text": "I have lived with type 1 diabetes for 23 years, and the thing that unsettled me in my early forties was not that the nights changed. It was that my interpretation of them stopped being reliable. Two decades of reading a 3am sweat as a hypo and being right almost every time — and then a stretch where I would treat it, and check afterwards, and find 6.2, having eaten glucose I did not need. That was the annoying direction."
      },
      {
        "t": "p",
        "text": "The frightening direction came later, once I had got comfortable with shrugging and going back to sleep, and one of those nights turned out to be 3.1. I had been wrong in both directions inside the same year, which is the specific fear I think most women in this position are carrying, so it seems worth saying out loud rather than tidying away."
      },
      {
        "t": "p",
        "text": "What changed is that I stopped interpreting. The meter lives on the bedside table, not in a bag in another room. I check first and think second, every time, including — especially — when I am sure. I keep an overnight low alarm on and let it be more annoying than I would ideally like. Once a month I look back over a fortnight of overnight traces with the waking episodes marked, which takes about ten minutes and has told me more than any amount of lying in the dark trying to decide what a sensation meant."
      },
      {
        "t": "p",
        "text": "That is one person’s routine, not a recommendation, and the routine is not really the point. The order is. Measure, then interpret. Everything downstream of that — what the pattern means, what to change, whether HRT belongs in the picture — is a conversation for your own team, and you will walk into it with something far better than a description."
      },
      {
        "t": "refs",
        "items": [
          {
            "claim": "Most people with type 1 diabetes do not wake up during a nocturnal hypoglycaemic episode.",
            "detail": "Schultes and colleagues induced hypoglycaemia overnight under polysomnographic monitoring in 16 people with type 1 diabetes and 16 age- and BMI-matched healthy controls, on separate nights against euglycaemic control conditions. With glucose lowered to a nadir of 2.2 mmol/L (the controls who woke did so between 2.2 and 2.7 mmol/L), only one of the 16 participants with type 1 diabetes awakened, compared with ten of the 16 controls (p = 0.001). Adrenaline rose about 7.5 minutes before waking in those who did wake, indicating that counter-regulation was occurring whether or not it produced arousal. It is a small laboratory study of induced hypoglycaemia rather than an observational one, which is a limitation — but the size of the difference is hard to explain away.",
            "cite": "PLoS Medicine · Schultes B et al., 2007;4(2):e69",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC1808097/"
          },
          {
            "claim": "Mild overnight lows track with worse sleep; the deepest and longest ones often go unnoticed.",
            "detail": "A prospective pilot study paired continuous glucose data with nightly subjective sleep ratings in 27 adults with type 1 diabetes (18 women, median age 27, median HbA1c 7.6%, median duration 16 years) across 573 analysed nights. Nights with glucose below 3.9 mmol/L — 23.9% of nights — scored significantly worse for sleep quality (3.3 vs 3.5, p = 0.03). The association ran with the frequency and duration of mild episodes rather than their depth: nights with a level-2 low below 3.0 mmol/L, on which glucose stayed below 3.9 mmol/L for a median of 150 minutes in total, showed no association with subjective sleep quality, and nor did the longest-duration nights. A small single-centre pilot, and the sleep measure is subjective by design.",
            "cite": "Scientific Reports · Gardner D, Tan HC, Lim GH, Zin Oo M, Xin X, Rama Chandran S, 2023",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC10684550/"
          },
          {
            "claim": "Impaired awareness of hypoglycaemia affects a substantial minority — and predicts severe hypos.",
            "detail": "A cross-sectional questionnaire study of 189 adults with type 1 diabetes (56.1% female, median age 41, median duration 19.3 years) found that 17.5% had impaired awareness of hypoglycaemia by the Gold score, meaning the usual early warning symptoms are muted or absent. Those with impaired awareness were more likely to report at least one severe hypoglycaemic episode in the previous 12 months — 37.5% against 11.7% (p = 0.001), about three times the rate; expressed as an odds ratio that is 4.5 (95% CI 2.0–10.9), which is odds rather than risk. Older participants were more likely to have it; in this cohort diabetes duration itself did not differ significantly between the two groups, so duration is not the whole story. It is a single-centre self-report study, and other cohorts report prevalence up to around 30% depending on the questionnaire used.",
            "cite": "PLoS One · Baxter F, Baillie N, Dover A, Stimson RH, Gibb F, Forbes S, 2024",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC11178233/"
          },
          {
            "claim": "Nocturnal hypoglycaemia clusters between midnight and 4am.",
            "detail": "A hierarchical cluster analysis of 2,519 overnight glucose series (midnight to 6am) from 395 adults with type 1 diabetes on multiple daily injections identified 16 distinct overnight patterns, six of them containing hypoglycaemic episodes. Episodes most commonly occurred between midnight and 4am, with later timing in traces that started from post-midnight hyperglycaemia. The authors conclude that bedtime glucose has limited value as a predictor — an overnight downward trend may matter more — and cite earlier studies identifying glucose variability and daytime hypoglycaemia as predictors of nocturnal lows.",
            "cite": "Journal of Personalized Medicine · Kladov DE, Berikov VB, Semenova JF, Klimontov VV, 2023",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC10608186/"
          },
          {
            "claim": "Roughly a third of people with type 1 diabetes show a nocturnal low followed by a rebound high.",
            "detail": "A cross-sectional retrospective analysis of 14 days of continuous glucose data from 755 adults with type 1 diabetes using FreeStyle Libre 2 (median age 49) found that 248 of them — 32.8% — had at least one episode of post-hypoglycaemic nocturnal hyperglycaemia. Those with the pattern spent longer above range, shorter in range, and had higher glucose variability than people whose nocturnal lows were not followed by a rebound. It is a retrospective sensor-data study, so it describes the pattern rather than establishing what should be done about it — and the authors could not tell whether the rebound came from counter-regulatory hormones or from carbohydrate taken to treat the low.",
            "cite": "Hormones (Athens) · González-Vidal T et al., 2025",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC12678521/"
          },
          {
            "claim": "Vasomotor symptoms affect 60–80% of women and peak in late perimenopause.",
            "detail": "Thurston and Joffe’s synthesis of the Study of Women’s Health Across the Nation reports that 60–80% of women experience vasomotor symptoms at some point in the menopausal transition, with occurrence and frequency peaking in late perimenopause and the early postmenopausal years around the final menstrual period. The review also documents associations with every component of disturbed sleep — sleep onset, sleep maintenance and early-morning waking. Prevalence varies substantially by racial and ethnic group.",
            "cite": "Obstetrics and Gynecology Clinics of North America · Thurston RC, Joffe H, 2011;38(3):489–501",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC3185243/"
          },
          {
            "claim": "When a hot flush and a hypo overlap, treat the low first.",
            "detail": "Breakthrough T1D’s patient guidance on menopause and type 1 diabetes states directly that some menopausal symptoms, including hot sweats and palpitations, feel similar to hypoglycaemia, and that where an episode is both a hypoglycaemic episode and a hot sweat you should always treat the low first. It also recommends checking glucose more often and points to continuous or flash monitoring, whose alarms warn when glucose is going too low or too high. This is a patient-facing resource rather than a primary study, cited here for the guidance itself.",
            "cite": "Breakthrough T1D · Menopause and type 1 diabetes",
            "url": "https://breakthrought1d.ca/life-with-t1d/menopause-and-type-1-diabetes/"
          }
        ]
      },
      {
        "t": "links",
        "title": "Related reading",
        "items": [
          {
            "name": "Perimenopause & diabetes: what’s really happening to your blood sugar",
            "url": "/blog/perimenopause-diabetes-blood-sugar/",
            "desc": "The pillar piece — how glucose actually behaves as oestrogen falls."
          },
          {
            "name": "Sleep and insulin resistance in women",
            "url": "/blog/sleep-insulin-resistance-women/",
            "desc": "What broken nights do to the next day’s glucose, whatever broke them."
          },
          {
            "name": "HRT and type 1 diabetes: what the evidence says",
            "url": "/blog/hrt-type-1-diabetes-evidence/",
            "desc": "The question most readers reach for next — whether treating the flushes is on the table."
          },
          {
            "name": "Your shoulder is stiffening and it probably isn’t age",
            "url": "/blog/frozen-shoulder-diabetes/",
            "desc": "Another midlife symptom that gets filed under hormones when diabetes is the stronger signal."
          },
          {
            "name": "CGM comparison: every sensor available in the UK",
            "url": "/cgm-comparison/",
            "desc": "If the overnight traces are the answer, this is how to choose the thing that draws them."
          }
        ]
      }
    ],
    "faq": [
      {
        "q": "How can I tell if my night sweats are perimenopause or low blood sugar?",
        "a": "Not from how they feel — and that is the honest answer rather than an evasive one. A falling glucose triggers an adrenaline response that produces sweating, palpitations, tremor and anxious waking, and a hot flush produces a strikingly similar set of sensations from a completely different starting point. The only thing that separates them at the moment it is happening is a glucose reading. There are three further reasons not to rely on symptoms: a substantial minority of people with type 1 diabetes have impaired awareness of hypoglycaemia and get no reliable warning at all; a low degrades the judgement you would use to assess it; and guessing generates no feedback, so a wrong call is never corrected and confidence grows without accuracy. Keep a meter within reach of the bed, check before you decide what the episode was, and use two to three weeks of overnight sensor traces to see whether the episodes cluster with lows or happen independently of them."
      },
      {
        "q": "Would I definitely wake up if I had a low blood sugar overnight?",
        "a": "No, and this is one of the more important things to know. In a controlled study published in PLoS Medicine, glucose was lowered overnight in 16 people with type 1 diabetes and 16 healthy controls under sleep monitoring: ten of the controls woke up, and only one of the participants with type 1 diabetes did. Waking is unreliable in both directions — it is why an overnight low alarm on a continuous or flash monitor is worth the irritation, and why “I slept through, so I must have been fine” is not a conclusion you can draw."
      },
      {
        "q": "What should I do if I wake up sweating and I’m not sure which it is?",
        "a": "Check your glucose first, then interpret. If it is low, treat it the way you and your diabetes team have already agreed and check again afterwards to confirm it has come back up. If it is in range, you have established that this particular episode was not a low, which is useful evidence for the vasomotor conversation. If it is both a low and a flush at once — which happens — the standard guidance is to treat the low first."
      },
      {
        "q": "Why do I wake up with a high blood sugar after a night I woke up sweating?",
        "a": "One possibility is a low earlier in the night followed by a rebound — from the body’s counter-regulatory response, from what you took to treat the low, or both. In a 2025 analysis of 14 days of sensor data from 755 adults with type 1 diabetes, around a third had at least one episode of a nocturnal low followed by hyperglycaemia. The morning high is a clue that something happened at 2 or 3am, so it is worth scrolling back through the overnight trace before assuming the evening dose was too small. What to change about it is a question for your diabetes team, not one to solve from a graph alone."
      },
      {
        "q": "Can a CGM actually tell perimenopause night sweats apart from hypos?",
        "a": "Over weeks, yes — in a single moment, only partly. A sensor lags blood glucose by several minutes and is least accurate when glucose is moving fast, so a reading taken mid-episode can mislead in the direction you least want. Where it is genuinely powerful is retrospectively: mark the nights you woke, look back across two or three weeks of overnight traces, and the pattern usually declares itself. Episodes that land on flat, in-range nights are not lows, and that is the finding worth taking to an appointment."
      }
    ]
  },
  "ua": {
    "cat": "Діабет і перименопауза",
    "imageAlt": "Жінка лежить без сну в ліжку вночі, поклавши руку на чоло.",
    "title": "Нічна пітливість чи гіпо? Як їх розрізнити, коли у вас і перименопауза, і діабет",
    "seoTitle": "Нічна пітливість чи гіпо? Як побачити різницю",
    "metaDesc": "Перименопауза й нічна гіпоглікемія дають майже однакову нічну пітливість. Як безпечно їх розрізнити — з досвіду 23 років із діабетом 1 типу.",
    "excerpt": "Перименопауза й нічна гіпоглікемія будять о третій ночі майже однаково, і за відчуттями їх не розрізнити. Ось що справді дає відповідь і чому важливий порядок дій.",
    "meta": "19 серпня 2026 · 9 хв читання",
    "lead": "Сайти про менопаузу скажуть вам, що нічна пітливість — це вазомоторний симптом. Сайти про діабет скажуть, що це ознака нічної гіпоглікемії. І ті, і ті написані для жінки, яка належить до однієї категорії. Якщо у вас діабет і ви десь у перименопаузі, ви не належите. Цей текст — про середину.",
    "blocks": [
      {
        "t": "quote",
        "text": "Ця стаття — освітній матеріал, а не медична порада, і вона свідомо не є переліком симптомів. Якщо ви прокинулися вночі з чимось, що може бути низьким цукром, спершу виміряйте глюкозу, а вже потім тлумачте — вимірювання займає секунди, і жоден текст, ні цей, ні будь-який інший, його не замінить. Тут немає жодних порад щодо доз. Як лікувати гіпоглікемію і чи щось міняти в інсуліні чи ліках — це розмова з вашою діабетичною командою."
      },
      {
        "t": "h",
        "text": "Проблема третьої ночі"
      },
      {
        "t": "p",
        "text": "Ви прокидаєтеся о третій ночі, і простирадло мокре наскрізь. Серце гупає. Вам жарко, потім холодно, потім руки ледь помітно тремтять — і до всього цього прив’язаний особливий різновид тривоги, який ніколи не переживає спробу описати його вдень. Поки ви встали, переодягли футболку й лягли назад, ви вже щось собі сказали: це менопауза, або це була гіпо, або це просто стрес."
      },
      {
        "t": "p",
        "text": "Оце речення — те, яке ви кажете собі до того, як щось перевірити, — і є вся проблема. Воно майже ніколи не стосується цього конкретного епізоду. Воно спирається на попередній і на те, який із двох ваших станів ви останнім часом подумки звинувачуєте."
      },
      {
        "t": "p",
        "text": "Є причина, чому «це, мабуть, менопауза» як відповідь за замовчуванням має вас насторожувати. Це відповідь, яка нічого не коштує — окрім тієї ночі, коли вона хибна, і тоді коштує дуже багато. Тому цей текст не починається з переліку того, чим ці два стани відрізняються на дотик. Він починається з того, що я скажу максимально прямо: здебільшого вони не відрізняються."
      },
      {
        "t": "h",
        "text": "Чому вони відчуваються так схоже"
      },
      {
        "t": "p",
        "text": "Коли глюкоза починає падати, організм запускає контррегуляторну відповідь, і найпомітніша її частина — адреналін. Адреналін дає пітливість, прискорене або гупаюче серцебиття, тремор, хвилю тривоги і — іноді — пробудження. Цей набір не є підписом низької глюкози. Це підпис адреналіну, а це зовсім інша річ."
      },
      {
        "t": "p",
        "text": "Приплив приходить з іншого боку, але задіює ту саму механіку. Коли естроген падає й коливається, звужується гіпоталамічна термонейтральна зона — той діапазон внутрішньої температури, у якому тіло почувається спокійно. Невелике підвищення, яке раніше пройшло б непоміченим, тепер пробиває стелю, і тіло починає скидати тепло: судини розширюються, вас кидає в жар, ви пітнієте, а потім вам стає холодно, бо ви проскочили в інший бік. Серцебиття й різке пробудження супроводжують це достатньо часто, щоб жінки описували все разом тими самими словами, якими описали б гіпоглікемію."
      },
      {
        "t": "p",
        "text": "Тобто те, що ви відчуваєте о третій ночі, — це відповідь організму на щось, а не саме це щось; і два різні тригери смикають один і той самий сигнал тривоги. Саме тому відчуття не може сказати вам про причину. Настанови Breakthrough T1D щодо менопаузи формулюють це так прямо, як рідко буває в пацієнтських матеріалах: припливи та серцебиття відчуваються схоже на гіпоглікемію, і там, де вони накладаються, гіпоглікемію завжди лікують першою."
      },
      {
        "t": "h",
        "text": "Що таке нічна гіпоглікемія насправді"
      },
      {
        "t": "p",
        "text": "Це падіння глюкози нижче цільового діапазону, поки ви спите: через пролонгований інсулін, через тренування раніше того дня, через алкоголь, через вечерю, яка була меншою або пізнішою за звичну, або через чутливість до інсуліну, яка тихо змінилася. Перименопауза теж належить до цього переліку, бо коливання естрогену й прогестерону зміщують чутливість до інсуліну — про це [базовий текст про те, що відбувається з цукром у крові, коли падає естроген](/ua/blog/perimenopause-diabetes-blood-sugar/)."
      },
      {
        "t": "p",
        "text": "Незручна частина — те, що буває далі. У контрольованому лабораторному дослідженні, опублікованому в PLoS Medicine 2007 року, глюкозу знижували вночі у 16 людей із діабетом 1 типу і 16 підібраних здорових учасників під повним моніторингом сну. З шістнадцяти учасників контрольної групи прокинулися десятеро. З шістнадцяти людей із діабетом 1 типу прокинулася одна. Не одна з трьох. Одна."
      },
      {
        "t": "p",
        "text": "Це означає, що «я б прокинулася, якби впала глюкоза» — небезпечна засновка. І дзеркальна теж: «раз я прокинулася, значить це була гіпо». Пробудження — поганий детектор в обидва боки."
      },
      {
        "t": "p",
        "text": "Є ранковий підпис, який варто знати: головний біль після пробудження, сон, що не відновив сили, плаский настрій і незвична втома наступного дня. Це підказка, а не діагноз. А в дослідженні 2023 року в Scientific Reports, де сенсорні дані щоночі поєднували з суб’єктивною оцінкою сну у 27 дорослих із діабетом 1 типу, зв’язок виявився в напрямку, який більшість вважає несподіваним: із поганим сном корелювали частота й тривалість м’якших гіпоглікемій, тоді як ночі, коли глюкоза опускалася нижче 3,0 ммоль/л, — а низька глюкоза тоді тривала загалом у середньому близько двох з половиною годин, — помітно не змінювали оцінку ночі взагалі. Наскільки погано це відчувалося і наскільки погано було насправді — це різні осі."
      },
      {
        "t": "h",
        "text": "Що таке перименопаузальна нічна пітливість насправді"
      },
      {
        "t": "p",
        "text": "Та сама історія терморегуляції, що й у денного припливу, з тією різницею, що ви були непритомна, коли він почався, — тож потрапляєте в його середину без розгону, який можна було б витлумачити. Вазомоторні симптоми зачіпають 60–80% жінок у певний момент переходу, з піком у пізній перименопаузі та в перші роки після останньої менструації, і вони пов’язані з усіма без винятку вимірами порушеного сну: засинанням, підтриманням сну і надто раннім пробудженням."
      },
      {
        "t": "p",
        "text": "Остання частина важлива для будь-кого з діабетом, бо розірваний сон не є нейтральним наслідком. Він підвищує інсулінорезистентність наступного дня — це окреме коло, про яке я писала в [тексті про сон та інсулінорезистентність](/ua/blog/sleep-insulin-resistance-women/)."
      },
      {
        "t": "h",
        "text": "Чому порівнювати симптоми — програшна стратегія"
      },
      {
        "t": "p",
        "text": "Варто пояснити, чому в цьому тексті немає порівняльної таблиці, бо «жодних відмінностей немає» було б перебільшенням і не тим, що я маю на увазі. Тенденції існують. Приплив частіше починається хвилею жару в грудях і обличчі, яка йде вгору й минає за кілька хвилин. Гіпоглікемія частіше дає голод і тремор і вщухає, коли цифра піднімається, а не сама собою. Проблема не в тому, що їх нічим не відрізнити. Проблема в тому, що це тенденції на рівні популяції, а ви застосовуєте їх до одного епізоду, у темряві, один раз."
      },
      {
        "t": "p",
        "text": "І ці тенденції лежать поверх спільної відповіді. Пітливість, прискорене серцебиття й різке тривожне пробудження — це те, що робить тіло, коли воно стривожене. Вони не приходять із позначкою, який саме сигнал смикнули. Будь-який їх перелік описує дзвін, а не руку на мотузці."
      },
      {
        "t": "p",
        "text": "Далі — незручний факт: попереджувальні симптоми доступні не всім однаково. Порушене відчуття гіпоглікемії, коли звичні ранні попередження приглушені або відсутні взагалі, було виявлене у 17,5% дорослих із діабетом 1 типу в опитувальному дослідженні 2024 року, а інші оцінки сягають близько 30%. Важливо, із чим це йде поруч: у тому ж дослідженні тяжку гіпоглікемію протягом попереднього року пережили 37,5% людей із порушеним відчуттям проти 11,7% тих, у кого воно збережене, — це приблизно втричі частіше, а шанси вищі вчетверо з половиною. Тобто симптоми найменш інформативні саме там, де наслідки найгірші. Перелік симптомів припускає, що сигналізація працює, — а в помітної частини тих, хто це читає, вона не працює."
      },
      {
        "t": "p",
        "text": "Подумайте також про те, коли ухвалюється рішення. Напівсонна, у темряві, о тій годині, коли мислення й так найгірше, — і якщо це гіпоглікемія, та сама здатність, якою ви оцінюєте ситуацію, псується тим, що ви оцінюєте. Падіння глюкози погіршує судження ще до того, як про себе оголосить. Ви просите зіпсований прилад виміряти поломку."
      },
      {
        "t": "p",
        "text": "Але найруйнівніше те, що вгадування нічого вас не вчить. Якщо ви вирішили, що це приплив, і заснули далі, вам ніхто не заперечить. Настає ранок, епізод позаду, і рішення тихо записується як правильне — просто тому, що не з’явилося нічого протилежного. Робіть так рік, і ваша впевненість зростатиме, а точність залишиться там, де була. І цього стану неможливо помітити зсередини. Я знаю, бо була в ньому. Єдине, що дає справжній зворотний зв’язок, — це цифра."
      },
      {
        "t": "p",
        "text": "А ще ці дві помилки коштують по-різному. Полікувати приплив, ніби це гіпоглікемія, коштує вам зайвої глюкози й трохи роздратування на себе. Прийняти гіпоглікемію за приплив і заснути далі — має дно, на яке ви не хочете натрапити. Коли помилки настільки нерівноцінні, метою перестає бути краще вгадування. Метою стає не потребувати його взагалі."
      },
      {
        "t": "h",
        "text": "Єдине, що надійно їх розрізняє"
      },
      {
        "t": "p",
        "text": "Цифра."
      },
      {
        "t": "p",
        "text": "Це вся відповідь, і я краще дам вам корисну версію, ніж приємну. У момент, коли це відбувається, лише вимірювання глюкози скаже, у якому з двох станів ви перебуваєте. Двадцять три роки практики його не замінюють. Наявність чи відсутність будь-якого конкретного відчуття — теж."
      },
      {
        "t": "p",
        "text": "Тож практична зміна до незручного проста: покладіть глюкометр на відстані витягнутої руки від ліжка. Причина, чому люди не перевіряють о третій ночі, майже ніколи не в скепсисі — вона в тому, що прилад унизу, а ви напівсонна, і вирішити легше, ніж піти. Спершу виміряти, потім вирішувати — саме в такому порядку, зокрема й у ті ночі, коли ви впевнені."
      },
      {
        "t": "p",
        "text": "Якщо цифра низька — лікуйте так, як ви вже домовилися зі своєю командою, і перевірте ще раз, щоб переконатися, що вона піднялася. Якщо цифра в діапазоні — ви дізналися дещо тверде: цей епізод не був гіпоглікемією. Не «мабуть, ні». Не був. І це вже факт, який можна принести на прийом."
      },
      {
        "t": "p",
        "text": "Одне застереження, якщо ви дивитеся на сенсор, а не на глюкометр. Сенсори вимірюють міжклітинну рідину й відстають від крові на кілька хвилин, і найменш точні вони саме тоді, коли глюкоза швидко рухається. Ця похибка несиметрична за наслідками: сенсор, який показує вас у діапазоні під час швидкого падіння, — це якраз те показання, якому довіряти найменше. Якщо симптоми й цифра не збігаються, підтвердіть глюкометром."
      },
      {
        "t": "h",
        "text": "Якщо це обидва одночасно"
      },
      {
        "t": "p",
        "text": "Так буде — у деякі ночі. Ці два стани не чергуються по черзі, і немає жодної причини, чому приплив не може випасти на ніч, коли глюкоза теж повзе вниз. Настанова тут однозначна й варта окремого рядка: спершу лікуйте гіпоглікемію. Приплив — це виснажливо, але не небезпечно. Нелікована гіпоглікемія — навпаки. Тут немає складного вибору."
      },
      {
        "t": "h",
        "text": "Що змінює в цьому питанні CGM"
      },
      {
        "t": "p",
        "text": "Глюкометр дає одну точку. Сенсор дає форму ночі — чи ви повзли вниз від опівночі, чи провалилися й відскочили, чи лінія була пласкою весь той час, поки ваше тіло займалося чимось зовсім іншим."
      },
      {
        "t": "p",
        "text": "З цього випливає дві речі. Перша — сигнали тривоги: нічний алярм на низьку глюкозу є прямою практичною відповіддю на те дослідження в PLoS Medicine. Якщо ваша власна реакція пробудження ненадійна, ви позичаєте надійну. Увімкніть його — і потерпіть, що він дратуватиме трохи більше, ніж вам хотілося б."
      },
      {
        "t": "p",
        "text": "Друга — те, що насправді дає відповідь, і вона ретроспективна. Візьміть два-три тижні нічних графіків і позначте ночі, коли ви прокидалися. Якщо епізоди збігаються з гіпоглікеміями — відповідь у вас є. Якщо вони припадають на пласкі ночі в діапазоні — відповідь інша. Якщо змішано (а в багатьох жінок у перименопаузі саме так), ви теж це знаєте, і це корисніше за будь-яку з чистих версій, бо означає: пояснення минулої ночі не можна перенести на цю."
      },
      {
        "t": "p",
        "text": "Але будьте чесні щодо того, чим є сенсор. Затримка й похибка при швидкій зміні реальні, як і хибні «низькі» від того, що ви на ньому спали. Це чудовий інструмент для пошуку закономірностей за тижні й посередній оракул в одну конкретну мить. Якщо ви обираєте, який брати, я підтримую [порівняння доступних CGM](/cgm-comparison-ua/) в актуальному стані, а [дослідник цукру в крові](/blood-sugar-investigator-ua/) проведе вас нічними патернами питання за питанням, якщо вам зручніше, коли ведуть, ніж читати таблицю."
      },
      {
        "t": "h",
        "text": "Як читати нічні графіки"
      },
      {
        "t": "table",
        "head": [
          "Що ви бачите вночі",
          "На що це зазвичай вказує"
        ],
        "rows": [
          [
            "Плавне рівне зниження після опівночі з провалом ще до 4-ї ранку",
            "Найпоширеніша форма нічної гіпоглікемії. Кластерний аналіз 2 519 нічних графіків від 395 дорослих із діабетом 1 типу показав, що більшість епізодів припадає саме на проміжок від опівночі до 4-ї. Це питання базального або вечірнього інсуліну до вашої команди — а не те, що коригують за статтею."
          ],
          [
            "Провал під ранок із подальшим підйомом: ви прокидаєтеся о третій, а на сніданок бачите високу цифру",
            "Постгіпоглікемічна нічна гіперглікемія — або контррегуляція, яка перестрибнула, або надмірне лікування самої гіпоглікемії; дослідження не змогло розрізнити, що саме. В аналізі 2025 року за даними сенсорів 755 дорослих із діабетом 1 типу приблизно у третини був щонайменше один такий епізод за два тижні. Ранкова висока цифра — підказка, що щось сталося раніше, і лікувати її, не подивившись, що їй передувало, — це саме та пастка."
          ],
          [
            "Пласка лінія в діапазоні всю ніч — у ніч, коли ви точно прокидалися",
            "Цей епізод не був гіпоглікемією. Якщо так повторюється в ночі, коли глюкоза в діапазоні, розмова переходить до вазомоторних симптомів і до сімейного лікаря чи менопаузального фахівця."
          ],
          [
            "Гіпоглікемії в ночі після важчих тренувань, після алкоголю або після меншої вечері",
            "Патерн із тригером, на який можна вплинути. Алкоголь зокрема здатен опустити глюкозу через багато годин після келиха — це тема, достатньо велика для окремого тексту. Принесіть цю пару вашій команді, а не розв’язуйте самі."
          ],
          [
            "Нічого послідовного — частина епізодів із гіпоглікеміями, частина без",
            "Найчастіша відповідь у реальному житті, і це не провал вправи. Це означає, що минулим епізодом не можна пояснити нинішній, — і саме тому варто продовжувати вимірювати."
          ]
        ]
      },
      {
        "t": "h",
        "text": "Що взяти з собою на прийом"
      },
      {
        "t": "p",
        "text": "Нічого з цього не є самодіагностикою. Це збір доказів, і різниця тут суттєва: думку лікаря змінює не кращий опис того, як епізоди відчуваються, а дані, які можна прочитати за тридцять секунд."
      },
      {
        "t": "table",
        "head": [
          "Що взяти",
          "Чим це допомагає"
        ],
        "rows": [
          [
            "Дати й час епізодів, хоча б приблизно",
            "Перетворює «я весь час прокидаюся мокра» на частоту й позицію на циферблаті — а це вже клінічне питання, а не скарга."
          ],
          [
            "Показник глюкози на момент кожного епізоду — і чи ви взагалі його отримали",
            "Найцінніша колонка. Епізод із зафіксованою цифрою в діапазоні є доказом так, як опис відчуття не буде ніколи."
          ],
          [
            "Чи щось змінило лікування гіпоглікемії",
            "Якщо ви полікували низьку глюкозу і симптоми вщухли, поки цифра піднімалася, це підтримує одне пояснення. Якщо ви полікували й не відчули різниці — скажіть і про це, воно інформативне в протилежний бік."
          ],
          [
            "Скриншоти нічних графіків за останні два-три тижні",
            "Показують закономірність, а не окремий випадок. Кілька показових ночей краще за підсумок, який ви написали самі."
          ],
          [
            "Контекст навколо кожного епізоду: тренування, алкоголь, час вечері, хвороба, день циклу, якщо цикл ще є",
            "Про це все одно запитають, а напівзгадані відповіді з’їдають більшу частину десятихвилинного прийому."
          ],
          [
            "Чи ви на МГТ, чи тільки думаєте про неї",
            "Це змінює обидві розмови одразу. МГТ може впливати на чутливість до інсуліну й поведінку глюкози, тож діабетична команда має про неї знати, навіть якщо призначав хтось інший."
          ]
        ]
      },
      {
        "t": "p",
        "text": "Якщо МГТ для вас на порядку денному, докази зібрані в [тексті про МГТ і діабет 1 типу](/ua/blog/hrt-type-1-diabetes-evidence/) — його варто прочитати перед прийомом, а не після."
      },
      {
        "t": "p",
        "text": "І та частина, про яку ніхто не попереджає: ваша діабетична команда і ваш сімейний лікар чи менопаузальний фахівець майже напевно не спілкуються між собою. Не через недбалість — це різні служби, різні записи, різні цикли візитів і направлення, у якому про нічну пітливість немає ні слова. На практиці ви — єдина людина, присутня в обох розмовах, а отже й місток, хотіли ви цієї роботи чи ні. Принесіть кожному з них той самий документ і прямо запитайте в кожного, що він хотів би, щоб знав інший."
      },
      {
        "t": "h",
        "text": "Що я роблю тепер"
      },
      {
        "t": "p",
        "text": "Я живу з діабетом 1 типу 23 роки, і те, що вибило мене з рівноваги на початку сорокових, — не те, що змінилися ночі. А те, що моє тлумачення цих ночей перестало бути надійним. Два десятиліття я читала пітливість о третій ночі як гіпоглікемію й майже завжди мала рацію — а потім настала смуга, коли я лікувала, перевіряла після, бачила 6,2 і розуміла, що з’їла глюкозу, яка була не потрібна. Це був дратівливий напрямок."
      },
      {
        "t": "p",
        "text": "Страшний прийшов пізніше, коли я вже звикла махнути рукою й заснути далі, — і одна з тих ночей виявилася 3,1. Я помилилася в обидва боки протягом одного року, і це саме той страх, який, здається, носить більшість жінок у такій ситуації. Тому про нього варто сказати вголос, а не прибрати подалі."
      },
      {
        "t": "p",
        "text": "Змінилося те, що я перестала тлумачити. Глюкометр живе на тумбочці, а не в сумці в іншій кімнаті. Спершу вимірюю, потім думаю — щоразу, і особливо тоді, коли впевнена. Тримаю увімкненим нічний алярм на низьку глюкозу й дозволяю йому дратувати більше, ніж мені хотілося б. Раз на місяць переглядаю два тижні нічних графіків із позначеними епізодами пробудження — це займає хвилин десять і дало мені більше, ніж будь-яка кількість лежання в темряві зі спробами вирішити, що означало відчуття."
      },
      {
        "t": "p",
        "text": "Це рутина однієї людини, а не рекомендація, — і річ узагалі не в рутині. Річ у порядку. Виміряти, потім тлумачити. Усе, що далі — що означає патерн, що змінювати, чи є тут місце для МГТ, — це розмова з вашою власною командою, і ви зайдете в неї з чимось значно кращим за опис."
      },
      {
        "t": "refs",
        "items": [
          {
            "claim": "Більшість людей із діабетом 1 типу не прокидаються під час нічної гіпоглікемії.",
            "detail": "Schultes і колеги викликали нічну гіпоглікемію під полісомнографічним моніторингом у 16 людей із діабетом 1 типу та 16 здорових учасників, підібраних за віком і ІМТ, в окремі ночі порівняно з еуглікемічними контрольними умовами. Коли глюкозу знижували до найнижчої точки 2,2 ммоль/л (учасники контрольної групи, які прокинулися, зробили це за 2,2–2,7 ммоль/л), прокинувся лише один із 16 учасників із діабетом 1 типу — проти десяти з 16 у контрольній групі (p = 0,001). У тих, хто прокидався, адреналін зростав приблизно за 7,5 хвилини до пробудження, тобто контррегуляція відбувалася незалежно від того, чи призводила вона до прокидання. Це невелике лабораторне дослідження з викликаною гіпоглікемією, а не спостережне, і це обмеження — але масштаб різниці важко пояснити інакше.",
            "cite": "PLoS Medicine · Schultes B та ін., 2007;4(2):e69",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC1808097/"
          },
          {
            "claim": "М’які нічні гіпоглікемії корелюють із гіршим сном; найглибші й найдовші часто залишаються непоміченими.",
            "detail": "Проспективне пілотне дослідження поєднало дані безперервного моніторингу з щонічною суб’єктивною оцінкою сну у 27 дорослих із діабетом 1 типу (18 жінок, медіана віку 27 років, медіана HbA1c 7,6%, медіана тривалості діабету 16 років) за 573 проаналізовані ночі. Ночі з глюкозою нижче 3,9 ммоль/л — 23,9% ночей — мали значуще гіршу оцінку сну (3,3 проти 3,5, p = 0,03). Зв’язок ішов через частоту й тривалість м’яких епізодів, а не через їхню глибину: ночі з гіпоглікемією 2-го рівня нижче 3,0 ммоль/л, коли глюкоза загалом трималася нижче 3,9 ммоль/л у медіані 150 хвилин, не були пов’язані із суб’єктивною оцінкою сну, як і ночі з найтривалішими епізодами. Це невелике одноцентрове пілотне дослідження, і оцінка сну в ньому суб’єктивна за задумом.",
            "cite": "Scientific Reports · Gardner D, Tan HC, Lim GH, Zin Oo M, Xin X, Rama Chandran S, 2023",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC10684550/"
          },
          {
            "claim": "Порушене відчуття гіпоглікемії є в помітної меншості — і воно передбачає тяжкі епізоди.",
            "detail": "Крос-секційне опитувальне дослідження 189 дорослих із діабетом 1 типу (56,1% жінок, медіана віку 41 рік, медіана тривалості діабету 19,3 року) показало, що у 17,5% було порушене відчуття гіпоглікемії за шкалою Gold — тобто звичні ранні попереджувальні симптоми приглушені або відсутні. Ті, у кого воно було, частіше повідомляли про щонайменше один епізод тяжкої гіпоглікемії за попередні 12 місяців — 37,5% проти 11,7% (p = 0,001), тобто приблизно втричі частіше; відношення шансів становить 4,5 (95% ДІ 2,0–10,9), і це саме шанси, а не ризик. Старші учасники мали його частіше; сама лише тривалість діабету в цій когорті між групами значуще не відрізнялася, тож справа не тільки в ній. Це одноцентрове дослідження на самозвітах, а інші когорти повідомляють про поширеність до приблизно 30% залежно від використаного опитувальника.",
            "cite": "PLoS One · Baxter F, Baillie N, Dover A, Stimson RH, Gibb F, Forbes S, 2024",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC11178233/"
          },
          {
            "claim": "Нічні гіпоглікемії групуються між опівніччю і 4-ю ранку.",
            "detail": "Ієрархічний кластерний аналіз 2 519 нічних глюкозних рядів (від опівночі до 6-ї) у 395 дорослих із діабетом 1 типу на багаторазових ін’єкціях виявив 16 різних нічних патернів, шість із яких містили епізоди гіпоглікемії. Найчастіше епізоди траплялися між опівніччю і 4-ю ранку, з пізнішим часом у графіках, які починалися з післяопівнічної гіперглікемії. Автори доходять висновку, що рівень глюкози перед сном має обмежену цінність як предиктор — нічний низхідний тренд може важити більше, — і посилаються на попередні дослідження, де предикторами нічних гіпоглікемій були варіабельність глюкози й денна гіпоглікемія.",
            "cite": "Journal of Personalized Medicine · Kladov DE, Berikov VB, Semenova JF, Klimontov VV, 2023",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC10608186/"
          },
          {
            "claim": "Приблизно в третини людей із діабетом 1 типу нічна гіпоглікемія переходить у відкатну гіперглікемію.",
            "detail": "Крос-секційний ретроспективний аналіз 14 днів даних безперервного моніторингу у 755 дорослих із діабетом 1 типу на FreeStyle Libre 2 (медіана віку 49 років) показав, що у 248 з них — 32,8% — був щонайменше один епізод постгіпоглікемічної нічної гіперглікемії. Ті, у кого був цей патерн, проводили більше часу вище діапазону, менше — в діапазоні, і мали вищу варіабельність глюкози, ніж люди, чиї нічні гіпоглікемії не переходили у відкат. Це ретроспективне дослідження сенсорних даних, тож воно описує патерн, а не встановлює, що з ним робити, — і автори не змогли визначити, чи відкат спричинили контррегуляторні гормони, чи вуглеводи, з’їдені для лікування гіпоглікемії.",
            "cite": "Hormones (Athens) · González-Vidal T та ін., 2025",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC12678521/"
          },
          {
            "claim": "Вазомоторні симптоми зачіпають 60–80% жінок і мають пік у пізній перименопаузі.",
            "detail": "Огляд Thurston і Joffe за даними Study of Women’s Health Across the Nation повідомляє, що 60–80% жінок переживають вазомоторні симптоми в певний момент менопаузального переходу, а їхня частота досягає піку в пізній перименопаузі та в перші постменопаузальні роки навколо останньої менструації. Огляд також фіксує зв’язок з усіма складовими порушеного сну — засинанням, підтриманням сну і раннім ранковим пробудженням. Поширеність суттєво різниться між расовими та етнічними групами.",
            "cite": "Obstetrics and Gynecology Clinics of North America · Thurston RC, Joffe H, 2011;38(3):489–501",
            "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC3185243/"
          },
          {
            "claim": "Коли приплив і гіпоглікемія накладаються, першою лікують гіпоглікемію.",
            "detail": "Пацієнтські настанови Breakthrough T1D щодо менопаузи та діабету 1 типу прямо зазначають, що деякі менопаузальні симптоми, зокрема припливи й серцебиття, відчуваються схоже на гіпоглікемію, і що коли епізод є одночасно і гіпоглікемією, і припливом, гіпоглікемію слід лікувати першою. Вони також радять частіше вимірювати глюкозу й згадують безперервний або флеш-моніторинг, сигнали якого попереджають, коли глюкоза стає надто низькою чи надто високою. Це пацієнтський ресурс, а не первинне дослідження, і наводиться тут саме заради самої настанови.",
            "cite": "Breakthrough T1D · Менопауза та діабет 1 типу",
            "url": "https://breakthrought1d.ca/life-with-t1d/menopause-and-type-1-diabetes/"
          }
        ]
      },
      {
        "t": "links",
        "title": "Дотичне читання",
        "items": [
          {
            "name": "Перименопауза та діабет: що насправді відбувається з вашим цукром у крові",
            "url": "/ua/blog/perimenopause-diabetes-blood-sugar/",
            "desc": "Базовий текст: як поводиться глюкоза, коли падає естроген."
          },
          {
            "name": "Сон та інсулінорезистентність у жінок",
            "url": "/ua/blog/sleep-insulin-resistance-women/",
            "desc": "Що розірвані ночі роблять із глюкозою наступного дня — хай там що їх розірвало."
          },
          {
            "name": "МГТ і діабет 1 типу: що каже доказова база",
            "url": "/ua/blog/hrt-type-1-diabetes-evidence/",
            "desc": "Питання, до якого більшість читачок переходить наступним: чи можна лікувати самі припливи."
          },
          {
            "name": "Плече дерев’яніє — і справа, найімовірніше, не у віці",
            "url": "/ua/blog/frozen-shoulder-diabetes/",
            "desc": "Ще один симптом середини життя, який списують на гормони, коли сильніший сигнал — діабет."
          },
          {
            "name": "Порівняння CGM: усі сенсори, доступні у Великій Британії",
            "url": "/cgm-comparison-ua/",
            "desc": "Якщо відповідь у нічних графіках, ось як обрати те, що їх малює."
          }
        ]
      }
    ],
    "faq": [
      {
        "q": "Як зрозуміти, що нічна пітливість — це перименопауза, а не низький цукор?",
        "a": "За відчуттями — ніяк, і це чесна відповідь, а не ухильна. Падіння глюкози запускає адреналінову відповідь із пітливістю, серцебиттям, тремором і тривожним пробудженням, а приплив дає разюче схожий набір відчуттів із зовсім іншої відправної точки. Єдине, що розрізняє їх у момент події, — вимірювання глюкози. Є ще три причини не покладатися на симптоми: у помітної частини людей із діабетом 1 типу порушене відчуття гіпоглікемії, тож надійного попередження немає взагалі; сама гіпоглікемія псує ту здатність судити, якою ви її оцінюєте; а вгадування не дає зворотного зв’язку, тож хибне рішення ніколи не виправляється, і впевненість зростає без точності. Тримайте глюкометр біля ліжка, вимірюйте до того, як вирішите, чим був епізод, і використайте два-три тижні нічних графіків, щоб побачити, чи збігаються епізоди з гіпоглікеміями, чи трапляються незалежно від них."
      },
      {
        "q": "Чи я обов’язково прокинуся, якщо вночі впаде цукор?",
        "a": "Ні, і це одна з найважливіших речей, які варто знати. У контрольованому дослідженні, опублікованому в PLoS Medicine, глюкозу знижували вночі у 16 людей із діабетом 1 типу і 16 здорових учасників під моніторингом сну: прокинулися десятеро з контрольної групи і лише одна людина з діабетом 1 типу. Пробудження ненадійне в обидва боки — саме тому нічний алярм на низьку глюкозу на безперервному чи флеш-моніторі вартий свого роздратування, і саме тому «я проспала, отже все було добре» — не той висновок, який можна зробити."
      },
      {
        "q": "Що робити, якщо я прокинулася мокра й не знаю, що це було?",
        "a": "Спершу виміряйте глюкозу, потім тлумачте. Якщо низька — лікуйте так, як ви вже домовилися зі своєю діабетичною командою, і перевірте ще раз, щоб переконатися, що вона піднялася. Якщо в діапазоні — ви встановили, що саме цей епізод не був гіпоглікемією, і це корисний доказ для розмови про вазомоторні симптоми. Якщо це і гіпоглікемія, і приплив одночасно — а так буває — стандартна настанова каже лікувати гіпоглікемію першою."
      },
      {
        "q": "Чому вранці високий цукор після ночі, коли я прокидалася мокра?",
        "a": "Один із варіантів — гіпоглікемія раніше вночі з подальшим відкатом угору: через контррегуляторну відповідь організму, через те, чим ви лікували гіпоглікемію, або через обидва. В аналізі 2025 року за 14 днями сенсорних даних 755 дорослих із діабетом 1 типу приблизно в третини був щонайменше один епізод нічної гіпоглікемії з подальшою гіперглікемією. Ранкова висока цифра — підказка, що щось сталося о другій чи третій ночі, тож варто прогорнути нічний графік назад, перш ніж вирішити, що вечірня доза була замалою. А що з цим робити — питання до вашої діабетичної команди, а не те, що розв’язують за графіком."
      },
      {
        "q": "Чи може CGM справді відрізнити перименопаузальну пітливість від гіпоглікемії?",
        "a": "За тижні — так; в одну конкретну мить — лише частково. Сенсор відстає від крові на кілька хвилин і найменш точний тоді, коли глюкоза швидко рухається, тож показання посеред епізоду може ввести в оману саме в тому напрямку, у якому найгірше. По-справжньому сильний він ретроспективно: позначте ночі, коли ви прокидалися, перегляньте два-три тижні нічних графіків — і закономірність зазвичай проявляється сама. Епізоди, що припадають на пласкі ночі в діапазоні, гіпоглікемією не були, і це саме той висновок, який варто нести на прийом."
      }
    ]
  }
};
